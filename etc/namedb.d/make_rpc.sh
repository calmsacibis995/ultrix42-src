#!/bin/sh
# @(#)make_rpc.sh	4.1	(ULTRIX)	7/2/90
#									
# 			Copyright (c) 1989-1990 by
# 		Digital Equipment Corporation, Maynard, MA		
# 			All rights reserved.				
# 									
# Usage:	make_rpc [-v] [etc_source rpc_dest]
...
#
# 12-Jun-89	logcher
#	Created script that takes a UNIX style rpc file and 
...
"
	;;
*)
	SOURCE=$NAMEDBDIR/$SRCDIR/rpc
	HESIOD=$NAMEDBDIR/rpc.db
...
SUBDOMAIN=rpc
START_KEY="; %RPC_START% - entries added by"
END_KEY="; %RPC_END%"
...
then
	...
	then
		...
		then
			...
			echo "usage: $PROG [etc_source rpc_dest]"
			eval "$QUIT"
		fi
	else
...
then
	...
	then
		...
		then
			echo "
The rpc database has already been installed but can not be
reconfigured automatically.  To change the current rpc configuration,
edit the file $HESIOD to remove the old rpc database
and run $PROG again."
			eval "$QUIT"
		else
			first_time="n"
