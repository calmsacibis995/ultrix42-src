#!/bin/sh
# @(#)make_protocols.sh	4.1	(ULTRIX)	7/2/90
#									
# 			Copyright (c) 1989-1990 by
# 		Digital Equipment Corporation, Maynard, MA		
# 			All rights reserved.				
# 									
# Usage:	make_protocols [-v] [etc_source protocols_dest]
...
#
# 12-Jun-89	logcher
#	Created script that takes a UNIX style protocols file and 
...
DEBUG)
	shift
	SOURCE=/tmp/protocols
	HESIOD=/tmp/protocols.db
...
"
	;;
*)
	SOURCE=$NAMEDBDIR/$SRCDIR/protocols
	HESIOD=$NAMEDBDIR/protocols.db
...
SUBDOMAIN=protocols
START_KEY="; %PROTOCOLS_START% - entries added by"
END_KEY="; %PROTOCOLS_END%"
...
then
	...
	then
		...
		then
			...
			echo "usage: $PROG [etc_source protocols_dest]"
			eval "$QUIT"
		fi
	else
...
then
	...
	if [ -z "$DOMAIN" ]
	then
		echo "BIND domain name not set.  Check $RESOLVC file."
		eval "$QUIT"
	fi
else
	DOMAIN=$3
...
then
	...
	then
		...
		then
			echo "
The protocols database has already been installed but can not be
reconfigured automatically.  To change the current protocols configuration,
edit the file $HESIOD to remove the old protocols environment
and run $PROG again."
			eval "$QUIT"
		else
			first_time="n"
