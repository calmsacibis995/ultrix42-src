#!/bin/sh
# @(#)make_group.sh	4.1	(ULTRIX)	7/2/90
#									
# 			Copyright (c) 1989-1990 by
# 		Digital Equipment Corporation, Maynard, MA		
# 			All rights reserved.				
# 									
# Purpose:	Script that takes a UNIX style group file and converts
#		it's info to Hesiod format.
# Usage:	make_group [-v] [etc_source group_dest]
...
#
# 12-Jun-89	logcher
#	Created script that takes a UNIX style group file and 
...
"
	;;
*)
	SOURCE=$NAMEDBDIR/$SRCDIR/group
	HESIOD=$NAMEDBDIR/group.db
...
SUBDOMAIN=group
START_KEY="; %GROUP_START% - entries added by"
END_KEY="; %GROUP_END%"
...
then
	...
	then
		...
		then
			...
			echo "usage: $PROG [-v] [etc_source group_dest]"
			eval "$QUIT"
		fi
	else
...
then
	...
	then
		...
		then
			echo "
The group database has already been installed but can not be
reconfigured automatically.  To change the current group configuration,
edit the file $HESIOD to remove the old group database
and run $PROG again."
			eval "$QUIT"
		else
			first_time="n"
