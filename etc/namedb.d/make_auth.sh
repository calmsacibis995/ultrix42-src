#!/bin/sh
# @(#)make_auth.sh	4.1	(ULTRIX)	7/2/90
#									
# 			Copyright (c) 1989-1990 by
# 		Digital Equipment Corporation, Maynard, MA		
# 			All rights reserved.				
# 									
# Purpose:	Script that takes a ULTRIX style auth file and converts
#		it's info to Hesiod format.
# Usage:	make_auth [-v] [etc_source hesiod_dest]
...
#
# 12-Jun-89	logcher
#	Created script that takes a ULTRIX style auth file and 
...
"
	;;
*)
	SOURCE=$NAMEDBDIR/$SRCDIR/auth
	HESIOD=$NAMEDBDIR/auth.db
...
SUBDOMAIN=auth
START_KEY="; %AUTH_START% - entries added by"
END_KEY="; %AUTH_END%"
...
then
	...
	then
		...
		then
			...
			echo "usage: $PROG [-v] [etc_source hesiod_dest]"
			eval "$QUIT"
		fi
	else
...
then
	...
	then
		...
		then
			echo "
The auth database has already been installed but can not be
reconfigured automatically.  To change the current auth configuration,
edit the file $HESIOD to remove the old auth database
and run $PROG again."
			eval "$QUIT"
		else
			first_time="n"
