#!/bin/sh
# @(#)make_networks.sh	4.1	(ULTRIX)	7/2/90
#									
# 			Copyright (c) 1989-1990 by
# 		Digital Equipment Corporation, Maynard, MA		
# 			All rights reserved.				
# 									
# Usage:	make_networks [-v] [etc_source networks_dest]
...
#
# 12-Jun-89	logcher
#	Created script that takes a UNIX style networks file and 
...
"
	;;
*)
	SOURCE=$NAMEDBDIR/$SRCDIR/networks
	HESIOD=$NAMEDBDIR/networks.db
...
SUBDOMAIN=networks
START_KEY="; %NETWORKS_START% - entries added by"
END_KEY="; %NETWORKS_END%"
...
then
	...
	then
		...
		then
			...
			echo "usage: $PROG [-v] [etc_source networks_dest]"
			eval "$QUIT"
		fi
	else
...
then
	...
	then
		...
		then
			echo "
The networks database has already been installed but can not be
reconfigured automatically.  To change the current networks configuration,
edit the file $HESIOD to remove the old networks database
and run $PROG again."
			eval "$QUIT"
		else
			first_time="n"
