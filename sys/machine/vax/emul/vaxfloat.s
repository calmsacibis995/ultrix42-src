...
.byte 0x88
 ...
 inst_polyf:				# entrance
	...
	movb	$1,poly_x_flag(fp)	# flag to denote a poly(f/d/g) inst 
 ...
 5:	movl	*reg_r3(fp),r0		# r0 = next coefficient
 	...
 	brw	4b			# start the next iteration
 ...
 inst_polyh:				# entrance
 	...
 	bbc	$psl_fpd,psl(fp),2f	# not instruction resumption - bypass
 ...
 1:	movq	(r11)+,r0		# r0,r1 = first part of argument
 	...
 	bsbw	unpack_float3		# unpack and save the value
 	cmpb	$31,reg_r4(fp)		# is the iteration count too large ?
 	blssu	3f			# no - skip
 	brw	7f			# processed the reserved operand
 2:	bsbw	read_access		# first operand is read only
 	bsbw	unpack_float3		# unpack and save the value
 	incl	op_index(fp)		# look at second operand
 	bsbw	read_access		# second operand is read only
 	cmpl	$31,r0			# is the value reserved ?
 	bgequ	4f			# no - skip
 3:	brw	operand_fault		# process the reserved operand
 ...
 # 
 #  If Polyx instruction, truncate to 32/64/64/128 bits, which, after
 #  1-bit left shift, will result in 31/63/63/127 bits, as required.
 #  If not Polyx, then skip to normalize.
	blbc	poly_x_flag(fp),norm

	caseb	*op_index(fp),$typ_f,$(typ_h-typ_f)
4:
	.word	5f-4b			# 6 - F_floating
	.word	6f-4b			# 7 - D_floating
	.word	6f-4b			# 8 - G_floating
	.word	7f-4b			# 9 - H_floating

5:	clrl	fraction1+8(fp)		# PolyF - clr 2nd longword
					# PolyF, PolyD, PolyG
6:	clrq	fraction1(fp)		# - clr 3rd/4th longwords
7:
norm:	bsbw	normalize		# normalize the result
	rsb
