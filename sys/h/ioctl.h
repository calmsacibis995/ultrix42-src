 ...
 *
 * 02-Aug-89 - Tim Burke
 *	Added DEVGETGEOM ioctl under the generic device information category.
 *	This ioctl is used to return device geometry information.
 *
 * 15-Aug-89 - Randall Brown
 *	Changed all references of TCSADFLUSH to TCSAFLUSH 
 * 
 * 27-Oct-89 - Tim Burke
 *	Added DKIOCEXCL to set/clear exclusive access to an MSCP disk.
 *
 * 10-Dec-89 - Matt Thomas
 *	Added SIOCMANREQ and SIOCGETEVENTS for HDLC support.  Added LIOCCONN
 *	to replace LIOCRES, LIOCSOL, and LIOCCMD.
 *
 *	Change definitions of _IO* to be independent of either K&R or ANSI
 *	C preprocessor semantics.
 *
 * 3-29-90 - Kuo-Hsiung Hsieh  
 * 	Added _POSIX_SOURCE to enclose non-posix symbols.
 *
 * 07-03-90	Fred L. Templin
 *	Added "SIOCIFRESET" and "SIOCEEUPDATE" functions
 *
 */
 ...
 */
#define _IOCPARM_MASK	0x7f		/* Parameters are < 128 bytes	*/
#define _IOC_VOID	(int)0x20000000	/* No parameters		*/
#define _IOC_OUT 	(int)0x40000000	/* Copy out parameters		*/
#define _IOC_IN		(int)0x80000000	/* Copy in parameters		*/
#define _IOC_INOUT	(int)(_IOC_IN|_IOC_OUT)
#define _IO(x,y)	(int)(_IOC_VOID|(x<<8)|y)
#define _IOR(x,y,t)	(int)(_IOC_OUT|((sizeof(t)&_IOCPARM_MASK)<<16)|(x<<8)|y)
#define _IOW(x,y,t)	(int)(_IOC_IN|((sizeof(t)&_IOCPARM_MASK)<<16)|(x<<8)|y)
#define _IOWR(x,y,t)	(int)(_IOC_INOUT|((sizeof(t)&_IOCPARM_MASK)<<16)|(x<<8)|y)
#endif /* _IO */
...
#define	TCSAFLUSH	_IOW('t',82,struct termios)/* Drain, flush, & set*/
...
#define LIOCCONN	_IOW('i', 44, struct ltattyi)	/* lat connect info */
...
/*
** 
**     X29 IOCTL definitions to augment the standard pty driver IOCTLs
*/

/*
 * The ioctl request code 'x' is defined for X.29 usage to set and read
 * the standard collection of X.29 parameters. Control of these parameters
 * is independent of the normal tty parameters.
 */
#define X29DSIZE  127
typedef struct X29PARAMS
{
  unsigned char x29_len;
  unsigned char x29_data[X29DSIZE - 1];		/* actual values       */
} x29params;

#define TIOCX29SET      _IOW('x',  1, x29params)     /* set x.29 params     */
#define TIOCX29GET      _IOWR('x', 2, x29params)     /* get x.29 params     */

/*
 * X.29 message types
 */
#define X29QD_IND        0             /* Indication of parameters      */
#define X29QD_INVBREAK   1             /* Invitation to break           */
#define X29QD_SET        2             /* Set parameters                */
#define X29QD_BREAKIND   3             /* Indication of break           */
#define X29QD_READ       4             /* Read remote parameters        */
#define X29QD_ERROR      5             /* Error message                 */
#define X29QD_SETREAD    6             /* Set and read remote params    */

#define X29QD_CONTROL  253	       /* Assume control of X.29 QD     */
#define X29QD_RLSE     254             /* Release control of X.29 QD	*/
#define X29QD_BADMSG   255	       /* Invalid X.29 QD message	*/
