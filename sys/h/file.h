 ...
 *									*
 *	DECwest ANSI mt 1990 July 03					*
 *	Changed								*
 *	"#ifdef KERNEL && !defined(_POSIX_SOURCE)" to			*
 *	"#if defined(KERNEL) && !defined(_POSIX_SOURCE)".		*
 *									*
 *	Paul Shaughnessy	28-Feb-1991				*
 *	Fixed GETF macro for a configurable number of open		*
 *	file descriptors.						*
 *									*
 ************************************************************************/
...
#if !defined(F_RDLCK) || defined(__POSIX)
struct flock {
	short	l_type;
	short	l_whence;
	off_t	l_start;
