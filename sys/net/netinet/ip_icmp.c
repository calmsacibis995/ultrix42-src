 ...
 *		Removed lk_in_ifaddr and lk_ifnet to coincide with
 *		slip changes.	
 *
 *	10-17-89 	Uttam Shikarpur					*
 *		Made MAXTTL a writeable entity, by introducing maxttl   *
 *									*
 *	28-May-89	Matt Thomas					*
 *		Fix panic on ICMP_INFOTYPE messages.  Also fix option	*
 *		probnlems in icmp_reflect()				*
 *									*
 *	27-Mar-89	U. Sinkewicz
 *		Lowered ipl of lk_rtentry, lk_ifnet, lk_in_ifaddr, as per
 *		lp changes made 3/16/89.
 *
...
#include "../net/netinet/icmp_var.h"
...
int	icmpprintfs = 0;
#endif
/* initialize maxttl to MAXTTL, defined in ip.h */
int maxttl = MAXTTL;
/*
	...
	struct in_addr dest;
{
	...
	if (oip->ip_off &~ (IP_MF|IP_DF))
		goto free;
	/*
	 * Pullup what we need into a single mbuf since the first
	 * mbuf could contain only the IP header, which results
	 * in a panic when attempting to point beyond end of data
	 * when we execute the ICMP_INFOTYPE macro or copy the old
	 * header.
	 */
	m = dtom(oip);
	/* Always need to see the stuff we're sending back */
	icmplen = oiplen + MIN(8, oip->ip_len);

	if (oip->ip_p == IPPROTO_ICMP)
		/* need to see first word (or less) of orginal ICMP */
		icmplen = MAX(icmplen, (oiplen + sizeof(long)));

	if (m->m_len < icmplen) {
		m = m_pullup(m, icmplen);
		if (m == 0) {
			return;
		}
	...
	nip->ip_off = 0;	/* keep DF flag for return packet */
	...
	struct ifnet *ifp;
{
	...
	ip->ip_src = t;
	ip->ip_ttl = maxttl;
	ip->ip_off = 0;	/* clear DF flag */

	if (optlen > 0) {
		/*
		 * Retrieve any source routing from the incoming packet
		 * and imerge in the other non-routing options.  Adjust
		 * the IP length.
		 */
		opts = ip_srcroute();
		if (!opts && (opts = m_get(M_DONTWAIT, MT_SOOPTS)))
			opts->m_len = 0;
...
#endif
	(void) ip_output(m, opts, (struct route *)0, 0, 0);
}
