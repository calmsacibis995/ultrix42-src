 ...
 *
 * 21-Jan-88  -- rafiey (Ali Rafieymehr)
 *	Created this header file for the Firefox driver.
 *
 **********************************************************************/

#ifdef KERNEL
#include "../io/uba/qevent.h"		/* include event struct defs */
#else
#include <io/uba/qevent.h>
#endif
 ...
 */
	...
	struct fgmap {			/* map of register blocks in Firefox */
...
struct fgcurstabupd {
	short x;			/* new curstab x value */
	short y;			/* new curstab y value */
	short hot_x;			/* new curstab hot_x value */
	short hot_y;			/* new curstab hot_y value */
	char flags;			/* bit0 => update x,
					   bit1 => update y,
					   bit2 => update hot_x,
					   bit3 => update hot_y */
	};

typedef struct _fg_commarea_desc {
    long int nsegs;
    struct  {
        long int nbytes;
        char *VA;
...
typedef struct _fg_timinginfo {
    unsigned short int csr;
    unsigned short int display_x;
    unsigned short int display_y;
    unsigned short int table_x;
    unsigned short int table_y;
    unsigned char table_width;
    unsigned char table_L1;
    unsigned char table_L2;
    unsigned char pipe;
    unsigned short int VBS;
    unsigned short int VBF;
    unsigned short int VSS;
    unsigned short int VSF;
    unsigned short int HBS;
    unsigned short int HBF;
    unsigned short int HSS;
    unsigned short int HSF;
    unsigned short int HS2;
    unsigned short int PVI;
    unsigned short int interrupt;
    } fg_timinginfo;

/*
 * constants for updating cursor, hot spots
 */

#define	UPDATE_X	1
#define	UPDATE_Y	2
#define	UPDATE_HOTX	4
#define	UPDATE_HOTY	8
...
#define CHAR_HEIGHT	21			/* char height in pixels */
#define CHAR_WIDTH	12			/* char width in pixels*/
...
#define	MAX_CUR_X	1280		/* max cursor x position */
#define	MAX_CUR_Y	1024		/* max cursor y position */
...
#define CURS_MIN_X 	216	/* device coordinate x = 0 */
