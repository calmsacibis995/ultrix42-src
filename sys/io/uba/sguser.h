 ...
 *	Fixed a bug in color_buf structure. The problem was only happening
 *	on eight plane machines.
 *
 * 07-Mar-87 -- vasudev (Vasudev K. Bhandarkar)
 *      Moved declarations of sg_next_fifo, sg_int_flag and change_section
 *      into shared memory (FIFOreq_header) so that these could be accessible 
 *      to user processes such as X.
 *
 * 18-Oct-87 -- tim (Tim Burke)
 *	Extended number of printable characters to 190 from 95.  This is being
 *	done for 8-bit support.
 *
 * 04-Aug-87 -- rafiey (Ali Rafieymehr)
 *	Moved sg_next_fifo declaration from "../qdddx/globals.h" to
 *	this file.
 *
 * 19-Mar-87 -- fred (Fred Canter for Ali Rafieymehr)
 *	X in the kernel changes.
 *
