#ifndef lint
static char *sccsid = "@(#)dh.c	4.1	ULTRIX	7/2/90";
#endif lint
 ...
 * 15-Dec-87 - Tim Burke
 *
 *	changed t_intrc to tp->t_cc[VINTR] so that this code will compile
 *	without errors in the wake of termio changes.  Note that this driver
 *	has not been updated to full termio compatibility with the other
 *	device drivers and will probably not work any more.
 *
 * 28-Jan-88 - Tim Burke
 *
 *	Changed references to t_ispeed and t_ospeed to use the POSIX cflag.
 *	This is needed because t_ispeed and t_ospeed have been removed from
 *	the tty structure.
 *	These changes have been made to allow this driver to COMPILE in light
 *	of the termio tty changes.  To date these changes are not complete for
 *	termio; nor have I been able to test them due to hardware unavailablity.
 */
...
#define OFLAG (OPOST)
#define CFLAG (PARENB|CREAD|CS7|ISPEED)
