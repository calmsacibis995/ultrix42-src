...
expr:	'(' expr ')' = { $$ = $2; }
	...
	| expr MCH expr	 = { $$ = (int) match((int) $1,(int)  $3); }
	| MATCH expr expr = { $$ = (int) match((int) $2,(int)  $3); }
...
#define EQL(x,y) !strcmp((char *)(x),(char *)(y))
long atol();
char	**Av;
int	Ac;
int	Argi;
...
#ifdef SUBR
/* subroutine by just two changes */
expr_main(argc, argv)
#else
main(argc, argv)
#endif SUBR
...
char *arith(op, r1, r2) char *r1, *r2; {
	...
	rv = malloc((unsigned)16);
	(void) sprintf(rv, "%D", i1);
	return rv;
}
...
char *Index(s, t) char *s, *t; {
	...
	for(i = 0; s[i] ; ++i)
		for(j = 0; t[j] ; ++j)
			if(s[i]==t[j]) {
				(void) sprintf(rv = malloc((unsigned)8), "%d", ++i);
				return rv;
			}
	return "0";
}
...
char *match(s, p)
{
	...
	(void) sprintf(rv = malloc((unsigned)8), "%d", ematch((char *)s, (char *)p));
	if(nbra) {
