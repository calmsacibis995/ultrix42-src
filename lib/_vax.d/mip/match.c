#ifndef lint
static	char	*sccsid = "@(#)match.c	1.3	(ULTRIX)	3/12/86";
#endif lint
...
* vdp003 - added support for doing f floating arithmetic in f float when
*	fflag is set. Uses the productions with need FLOATFLG. Also 
* 	added qdebug which prints out the table production the was matched
*	
*	Rich Phillips  8-17-84
* RAP002  Add code to put out code for UNARY CALL's to _asm.
*
*	Rich Phillips  7-11-84
* RAP001  Print an error message when the register number in tshape
*	  is out of bounds. If it happens, its probably because oreg is
*	  changed to a register when a structure is passed as a parameter
*	  for some reason (see genargs in order.c).
*
*************************************************************************/
# include "mfile2"
...
extern qdebug;		/*vdp003 print out the code template matched*/
extern vdebug;
extern int fflag; 	/*vdp003 floating point */ 
int fldsz, fldshf;
...
# endif
	...
	case REG:
		...
		*/
		...
		/* RAP001 */
		if (p->tn.rval >=REGSZ ) 
			cerror ("incorrect register number in tshape");
...
match( p, cookie ) NODE *p; {
	...
	for( ; q->op != FREE; ++q ){
		...
		/* No match if floating point flag is not set and we have a 
		 * float production (i.e. FLOATFLG)  -ugh-  vdp003  
		 */ 
		if ((!fflag) && (q->needs & FLOATFLG) )
			 continue;  
		...
		if (qdebug)
		        { 	
			/* vdp003- print out code template matched -
			 * it's not pretty, but it helps
			 */
			printf("Template matched: "); 
			printf("%s", opst[q->op]); 
			printf("	visit: "); /*, q->visit);*/
			prcook(q->visit);  
			printf("\nleft shape - %o	left type - %o\n", q->lshape, q->ltype);
			printf("right shape - %o	right type - %o\n", q->rshape, q->rtype); 
			printf("needs - %o 	rewrite -%o\n",q->rewrite); 
			printf("cstring - %s\n", q->cstring); 
		}
#endif BUG1
...
expand( p, cookie, cp ) NODE *p;  register char *cp; {
	...
	for( ; *cp; ++cp ){
		...
		case 'R':
			/* RAP002
				Put out the asm information that was saved
				in scan.c if this is a call to asm.
			*/
			if ( !strcmp (p->in.left->in.name,"_asm") ){
				outasm(p->in.left->in.asminfo);
				return;
				}
			continue;
		case 'T':
