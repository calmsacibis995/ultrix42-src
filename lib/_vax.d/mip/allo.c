#ifndef lint
static	char	*sccsid = "@(#)allo.c	1.3	(ULTRIX)	3/12/86";
#endif lint
...
*
*	004	Lu Anne Van de Pas, 2-MAR-86
*		Added support for f floating arithmetic to be done in 
*		float instead of being converted up to double when the 
*		fflag is set
*
*		Rich Phillips 13-Sept-84
*	003	Get rid of nflags, change that needed it is being removed.
*
*		Rich Phillips  20-Aug-84
*	002	Copy asminfo (and nflags) when copying a node
*
*		Rich Phillips  6-July-84
*	001     Allocate 2 registers for relation operators that require
*		a conversion from float to double of one of their children.
*		The problem was that szty (called from usable) checks the
*		type of the operator, but that is integer for relation
*		operators. See OPLOG entries in table.c where one child
*		is float and the other is double. 
*
*		This fixes the bug ff[i] >= dd[j].
*
****************************************************************************/
# include "mfile2"
...
allo( p, q ) NODE *p; struct optab *q; {
	...
	n = q->needs;
	if (n & FLOATFLG)	/*vdp004 - we are done with FLOATFLG*/
		n &= ~FLOATFLG;	/*vdp004 turn off*/  
...
/*  chkcond call added for fix RAP001 */
	if( (n&NAMASK) && ((szty(p->in.type) == 2 || chkcond(p) ) ) ){ /* only do the pairing for real regs */
		if( r&01 ) return(0);
...
/*   RAP001
     ...
     The type of descendants is not brought up for condition nodes. Check
     if we are setting up for a conversion from float to double, if so
     return 1 so 2 registers will be allocated.
*/
chkcond( p ) NODE *p; {

	if ( p->in.op >= EQ && p->in.op <= UGT )
		if ( ( p->in.right->in.type == FLOAT && p->in.left->in.type
			== DOUBLE )  ||
		     ( p->in.right->in.type == DOUBLE && p->in.left->in.type
			== FLOAT ))
			return(1);
	return(0);
}
	...
	*qq = NIL;
	...
	case REG:
		if( !rtyflg ){
			...
			else if ((! fflag) & ( p->in.type == FLOAT )) p->in.type = DOUBLE; 	/*vdp004 don't rewrite node to be of type double if fflag */ 
			}
...
ncopy( q, p ) NODE *p, *q; {
	...
	q->tn.lval = p->tn.lval;
	q->tn.rval = p->tn.rval;
	q->in.asminfo= p->in.asminfo;	/*RAP002*/
#ifdef FLEXNAMES
	q->in.name = p->in.name;
#ifdef ONEPASS
