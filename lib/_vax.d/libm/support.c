...
#ifdef VAX
        if( (k= *px & mexp ) != ~msign ) {
            if( N<-260) return(0.0); else if(N>260) return(HUGE_VAL);
#else   /* IEEE */
        if( (k= *px & mexp ) != mexp ) {
            if( N<-2100) return(0.0); else if(N>2100) return(HUGE_VAL);
            if( k == 0 ) {
            ...
            if((k = (k>>gap)+ N) > 0 )
                ...
                else {errno = EDOM; x=HUGE_VAL;}       /* overflow */
            else
                if( k > -prep1 ) 
                                        /* gradual underflow */
...
/* Always guard against a reserved operand on vax. sin(-M_PI) cld IPO 04290 */
