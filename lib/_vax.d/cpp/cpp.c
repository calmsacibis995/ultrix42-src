#ifndef lint
static char *sccsid = "@(#)cpp.c	4.1	ULTRIX	7/3/90";
#endif lint
 ...
 *
 *	Jon Reeves, 25-Apr-89
 * 013- Add new features from MIPS V2.0 beta: Pascal comment handling
 *	(if LANGUAGE_PASCAL defined); pragma passing; -mips2 flag passed
 *	Some source recorrelating: 006's INCLDIR is now MAXIDIRS
 *
 *	A note for posterity, since it almost changed here: macro names
 *	cannot be allowed to start with $, since it makes cpp unusable with
 *	assembler immediate constants (like libc syscall jackets).
 *
 *	Jon Reeves, 31-Oct-88
 * 012- Fix eob handling; if eob fired at the end of a // comment, we
 *	infinite looped.
 *
 *	Jon Reeves, 26-Sep-88
 * 011-	Merge in features from MIPS cpp (-I<null>; ignore -v).
 *
 *	Jon Reeves, 14-Jan-88
 * 010- Up 'symsiz' (the size of the symbol table for define constants)
 *	from 2500 to 5000.  (Contrary to comments on change 8, that
 *	change only removed restrictions on the side buffer.  Change 5
 *	is still obsolete.)
 *
 *	Jon Reeves, 03-Dec-87
 * 009-	Merge mod.sources v07i023 patches: add #elif (code from Doug
 *	Gwyn) and, if the -B option is present, recognize //.*\n
 *	comments for C++ (code by Arnold Robbins, Emory U.).
 *	Corrections for line number handling by jlr.
 *
 *	Jon Reeves, 28-May-87
 * 008- Merge BSD 4.3 changes, including: removal of arbitrary limits
 *	(makes 005 obsolete); detect unclosed comments; recognize #ident
 *	for V.3 compatibility; handle # line "file" better; -M same as -Em
 *	(subsumes first part of 004).
 *
 *	Jon Reeves, 27-May-87
 * 007- Copy newlines appearing between macro name and opening paren.
 *	Keeps dbx line counts happy.
 *
 ...
 *   University of California, Berkeley, MIPS Computer Systems, Inc.,	*
 *   and from Bell Laboratories.  Use, duplication, or disclosure is	*
 *   subject to restrictions under license agreements with University	*
 *   of California, MIPS Computer Systems, Inc., and with AT&T.		*
 *									*
	  ...
	  for (;;) {
		if (*p=='/' && eolcom) {
			...
			for (;;) {
				...
				} else p+=2; /* jlr012 ignore null byte */
			}
		endcpluscom:
			if (!passcom) {outp=inp=p; --flslvl;}
			incomment--;
			goto prevlf; /* AR's "newline"->++lineno jlr009 */
			break;
		}
...
# endif
	...
	case '\n': {
/* newline: jlr009 -- not needed with fix to AR's code */
	...
	*--inp=SALT; outp=inp; ++flslvl; np=slookup(inp,p,0); --flslvl;
	if (np==defloc) {/* define */
		...
		if (flslvl)
		{
			...
			{
				ifdone[trulvl++] = 1;
/*	Yyparse() finished this line.  Back up.  jlr009 */
				--lineno[ifno];
				sayline (CONT);
				++lineno[ifno];
			}
