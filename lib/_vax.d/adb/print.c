#ifndef lint
static char *sccsid = "@(#)print.c	4.1	ULTRIX	7/3/90";
#endif lint
 ...
 *	Added a new $ command ($x) to print out the proc. address of the
 *	last process running on the specified processor. The processor 
 *	number is given in the address field
 *
...
printtrace(modif)
{
	...
	switch (modif) {
	    ...
	    case 'x':   /* get info about a processor */
		if (kernel == 0) 
			printf("not debugging kernel\n");
		else {
			/* if there is a paramater sent in then
			   print out the state of that processor
			   and put us in that context */
			if (adrflg) {
				int pte;
				extern struct cpudata cpu_data;

				/* change to new context if valid cpu */
				if (print_cpudata(adrval)) { 
			
					/* read up the proc for this cpu */
					lseek(fcor,vtophys(cpu_data.cpu_proc),0);
					read(fcor,&newproc,
							sizeof(struct proc));

					pte = access(RD,newproc.p_addr,DSP,0);
					masterpcbb = (pte&PG_PFNUM)*512;
					getpcb();
				}
			} else {
