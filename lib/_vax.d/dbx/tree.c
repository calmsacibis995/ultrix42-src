...
Operator op;
{
	...
	case O_PRINTF:
	    p->value.printform.clist = nextarg(Node);
	    p->value.printform.lang = nextarg(Language);
	    break;
...
Node exp;
{
    ...
    if(exp->op == O_VREG and 
		((streq(ident(exp->nodetype->name), "$vmr")) ||
			(streq(ident(exp->nodetype->name), "$vcr")) ||
				(streq(ident(exp->nodetype->name), "$vaer")) ||
					(streq(ident(exp->nodetype->name), "$vlr")))) {
				error("illegal vector register syntax");
	}
    if(exp->op == O_SYM and isreg(exp->nodetype)) {
		error("illegal register syntax");
	}
	if (exp->op == O_RVAL) {
	p = exp->value.arg[0];
	dispose(exp);
...
Node p;
{
	...
	case O_INDIR:
	    ...
	    {
	    	beginerrmsg();
	    	fprintf(stderr, "can not take address of \"");
	    	prtree(stderr, p);
	    	fprintf(stderr, "\"");
	    	enderrmsg();
	    }
	...
	case O_SYM:
		...
		else
		{
	    	...
	    	} else if (isblock(p->value.sym)) {
	    		beginerrmsg();
	    		fprintf(stderr, 
				   "can not take address of module or program, please qualify");
	    		enderrmsg();
	    	} else {
...
Command cmd;
{
	...
	case O_PRINTF:
	    if (cmd->value.arg[1] != nil) {
			switch (language_suffix(cmd->value.printform.lang)[1]) {
				default:
				case 'c': fprintf (f, "printf "); break;
				case 'p': fprintf (f, "writeln "); break;
				case 'f': fprintf (f, "write "); break;
			} 
	    } 
	    prtree(f, cmd->value.arg[0]);
	    break;
