 ...
 *									*
 *	005 - Made # and \ work as expected in shellmode.		*
 *	      (Jon Reeves, January 21, 1989)				*
 *									*
 *      004 - Added a <cr> for the -x option for use with xdb           *
...
/* derived from: "@(#)scanner.c	5.1 (Berkeley) 5/31/85" */
#ifndef lint
static char sccsid[] = "@(#)scanner.c	4.2	ULTRIX	11/9/90";
#endif not lint
...
public boolean xdb;		/* xdb specific features */
...
private File in;
private File recorded_input;
private char *record_name = nil;
...
public set_record(string)
char *string;
{
	if(record_name != nil) {
		fclose(recorded_input);
		record_name = nil;
	}
	if(strlen(string)) {
		if((recorded_input = fopen(string, "w")) != nil) {
			record_name = strdup(string);
		} else {
			error("unable to open %s for recording", string);
		}
	}
}
...
#   endif
	if(t == DELETE) {
		...
		if(strncmp((tmp = (char *)strpbrk(line, "a")), "all", 3) == 0) {
			tmp[0] = '*';
			tmp[1] = ' ';
			tmp[2] = ' ';
		}
