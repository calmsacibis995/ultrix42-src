 ...
 *									*
 *  007 - fixed 'print function()' and "can't set a breakpoint in *
 *		  a function and do a 'print function(). also, fixed the  *
 *		   function return value to be correct.					  *
 *		  (L Miller, 25JAN90)									*
 *									*
 *  006	- Added support for vectors.							*
 *		  (L Miller, 18JAN90)									*
 *									*
 *	005 - Re-merged 4.3 changes to fix: a) dump of variables in	*
 *	      unnamed blocks was useless ("the mysterious $b" in 001;	*
 *	      b) $unsafecall wasn't there though it was documented.	*
...
#ifndef lint
static char sccsid[] = "@(#)runtime.c	2.1	ULTRIX	4/24/89";
#endif not lint
...
#define isstackaddr(addr) \
    (((addr) < 0x80000000) and ((addr) > USRSTACK))
