 ...
 *	005 - Remove 004.  A bad idea, since the path name goes into	*
 *	      the symbol tables, and may be the only way to tell two	*
 *	      files apart.  As coded, 004 made it impossible to stop in	*
 *	      any file not compiled in '.'.  (jlr, April 11, 1988)	*
 *									*
 *	004 - Only look for the base part of the name in the a.out.	*
 *	      (jlr, March 17, 1988)					*
 *									*
 *	003 - Re-merged 4.3 changes.  (Jon Reeves, May 14, 1987)	*
 *									*
...
#ifndef lint
static char sccsid[] = "@(#)mappings.c	4.2 Ultrix 11/9/90";
#endif not lint
...
#define ADDRNOEXEC ((Address) -2) /* line number is not executable */
...
String name;
{
	...
	if(line > linetab[start].line && line < linetab[end].line)
		return ADDRNOEXEC;
	else
    	return NOADDR;
}
