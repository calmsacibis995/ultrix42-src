 ...
 *									*
 *  005 - Fixed erroneous type mismatch.		*
 *		  (Lee Miller, Jan 24, 1990)			*
 *									*
 *	004 - Changed c_typematch to accept 'integer' as well as	*
 *	      'int' because of lk brain damage.				*
 *	      (Jon Reeves, June 9, 1988)				*
 *									*
...
#ifndef lint
static char sccsid[] = "@(#)c.c	2.1	ULTRIX	4/24/89";
#endif not lint
...
public c_init()
{
    ...
    language_setop(langC, L_PRINTF, c_printf);
}
...
Symbol type1, type2;
{
	...
	b = (Boolean) (
		...
		t2->language == primlang     *lrm*/
	    ) or (
		t1->class == ARRAY and c_typematch(t1->type, t_char) and
		t2->class == ARRAY and c_typematch(t2->type, t_char) /*and
		t2->language == primlang     *lrm*/
	    ) or (
		isrange(t1, "integer") and		/* jlr004 */
		(t2 == t_int->type or t2 == t_char->type)
	    )
	);
    }
...
public Node c_printf(p, argv)
Node p;
char **argv;
{
	char str[MAXSTRLENGTH];
	char *fmt;
	va_list ap = (va_list)&argv[1];

	fmt = argv[0];
	if (!fmt) return;
	while (*fmt) {
	    if (strncmp(fmt,"%s",2) == 0)
		error("percent-s format not implemented for printf");
	    fmt++;
	} /* while */
	vsprintf(str, argv[0], ap);
	fputs(str, stdout);
}
