 ...
 *
 * 001	Rich Phillips, 26-July-1984
 * 	Don't issue a warning for byte or word displacements to global symbols
 ...
 ***********************************************************************/
#ifndef lint
static char sccsid[] = "@(#)ascode.c	4.3 ULTRIX 9/4/90";
#endif /* not lint */
	...
	int		nact;
{
	...
	if (passno == 1) {
		 ...
		 */
		switch( ((atype = fetcharg(ip, i-1)) & ACCESSMASK) >> TYPLG){
		case ACCI >> TYPLG:
		case ACCR >> TYPLG:
			break;
		case ACCB >> TYPLG:
	...
	if (jxxflg < 0)
		ijxout(opcode, ap, nact);      /* IJXXX */
	else 
        putins(opcode, ap, nact);      /* INST0, INSTn */
}
	...
	int	n;			/* Must be positive */
{
	...
	for (i=0; i<n; i++,ap++) {	/* some args take more than 1 byte */
	     ...
	     */
	    switch (argtype&~(AINDX|ASTAR|AVECT)) {
		case AREG:
		case ABASE:
		case ADECR:
		case AINCR:
		...
		case ACTRL:
			/* generate a word literal for cntrl word.*/
			argtype = fetcharg(ITABFETCH(opcode), i);
			argtype &= TYPMASK;
			dotp->e_xvalue += ty_nbyte[argtype];
			break;
...
int immconstant(xp, argtype, valuep, vect_flg)
	reg	struct	exp	*xp;
		int	argtype;
		int	*valuep;
        int     vect_flg;
