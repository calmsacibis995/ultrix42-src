#ifndef lint
static	char	*sccsid = "@(#)asscan2.c	4.3	(ULTRIX)	9/4/90";
#endif /* lint */
	...
	struct tokbufdesc *bufferbox;
{
	...
	union   Vinst_mod vmod;         /* vinst modifier value */
   ...
   loop:
	...
	case DIV:		/*process C style comments*/
		if ( (ch = getchar()) == '*') {  /*comment prelude*/
			...
			goto ret;
		} else if ((val == VINST0) || (val == VINSTn)) {
		  /* vector modifier */
		  ryylval = 0;
		  for (i = 1; i <= 2; i++) {
		    if (INCHARSET(ch, MODIFIER)) {
		      vmod.mod_value.mod_length = i;
		      if (INCHARSET(ch, ALPHA))
			ch |= 0100; /* convert to lower */
		      vmod.mod_value.mod_ch[i-1] = ch;
		      ch = getchar();
		    } /* end if */
		    else 
		      ungetc(ch);
   ...
   ret:	
	...
	switch(val){
		...
		case	VINST0:  
		case	VINSTn:  
				pptr(bufptr, (int)(struct symtab *)ryylval);
				break;
        case    VMODIFIER:
				pint(bufptr, ryylval);
				break;
