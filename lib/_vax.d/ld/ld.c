...
#ifndef lint
static char *sccsid = "@(#)ld.c	4.4	ULTRIX		9/17/90";
#endif
 ...
 *									*
 * 014	Ken Lesniak, 05-Dec-89						*
 *	Added Bflag to tell ld to force BSS to start on a 512 byte	*
 *	(hardware page) boundary.					*
 *									*
 * 013	Mark Parenti, 08-Jun-88						*
 *	Changed signal handlers to void.				*
 *									*
 * 012	Jon Reeves, 24-Nov-87						*
 *	Add logic for execution mode (a_mode field).			*
 *									*
 * 011	Jon Reeves, 05-Aug-87						*
 *	Don't scribble on start of string table when the link includes	*
 *	a null file by name.						*
 *									*
 * 010	Jon Reeves, 01-Jun-87						*
 *	Incorporate 4.3 changes (mostly -L option)			*
 *									*
 * 009	Jon Reeves, 01-Jun-87						*
 *	Don't make -r output executable unless -d present.  Avoids	*
 *	user errors and increases X/OPEN compliance.			*
 *									*
 ...
 *	Add 4.3 speed ups (subsumed in 010)				*
 *									*
 ...
 * 001- Fix some dereferencing of NULL pointers (partially subsumed in	*
 *	010).								*
 *									*
 ************************************************************************/
...
int	Bflag;		/* set if want 512 byte alignment on BSS */
...
enum	Mode {
	sys_v, posix, bsd}
	ex_mode = bsd;	/* 012 - execution mode; changed by -Y/PROG_ENV */
...
char **argv;
{
	...
	if ( strcmp("SYSTEM_FIVE",pep=getenv("PROG_ENV")) == 0)
		ex_mode = sys_v;
	else if ( strcmp("POSIX",pep) == 0)
		ex_mode = posix;
	else if ( strcmp("BSD",pep) == 0)
		ex_mode = bsd;
	...
	for (c=1; c<argc; c++) {
		...
		case 'Y':
			if (ap[++i] == '\0')
				ex_mode = sys_v; /* For pre-V2.4 compatibility */
			else if (strcmp("SYSTEM_FIVE", &ap[i]) == 0)
				ex_mode = sys_v;
			else if (strcmp("POSIX", &ap[i]) == 0)
				ex_mode = posix;
			else if (strcmp("BSD", &ap[i]) == 0)
				ex_mode = bsd;
			else
				error(-1, "warning: bad -Y argument");
			goto next;
		case 'L':
			goto next;
		default:
...
delexit()
{
	...
	if (delarg==0 && (arflag==0 || dflag) && Aflag==0)	/*009*/
...
/*	Even though there's nothing here, we still mkfsym the name.  011 */
			if (libflg == 0) /* 011 */
				ssize += sizeof cursym; /* 011 */
			return (0);
		}
		error(1, "no namelist");
	}
...
middle()
{
	...
	/*
	 * If requested, start BSS on a 512 byte hardware page boundary
	 * (the 'pagesize' used in the rest of ld is the software page size).
	 * This code causes the data size to be padded out to a 512 byte
	 * interval.  BSS is started directly after data, and may still be
	 * included in data (not bss) since data (later in ld) is rounded
	 * up to a 'pagesize' boundary.
	 */
	if (Bflag)
		dsize = round(dsize, 512);
