 ...
 *
 * 001-  Update arraysz to prevent too many calls to realloc, and
 * 	 do an fstat in case the directory has grown.
 *
 ***********************************************************************/
	...
	int (*select)(), (*dcomp)();
{
	...
	while ((d = readdir(dirp)) != NULL) {
		...
		if (++nitems >= arraysz) {
			/*RAP001
				See if the directory has grown and update
				arraysz. Also, use arraysz in call to realloc.
			*/
			if (fstat(dirp->dd_fd, &stb) < 0)
				return(-1);
