...
register struct utmp *entry;
{
	extern struct utmp ubuf;
	struct utmp *getutent();
/*	register short type;	/* BRL -- no longer used */

/* Start looking for entry.  Look in our current buffer before */
/* reading in new entries. */
	do {

	/* BRL -- the following section has been rewritten for older UNIXes: */

/* If there is no entry in "ubuf", skip to the read. */
		if ( strcmp( ubuf.ut_line, EMPTY ) != 0 )
			{
/* Do not look for an entry if the user sent us an EMPTY entry. */
			if ( strcmp( entry->ut_line, EMPTY ) == 0 )
				return NULL;

/* For BOOT_MSG, OTIME_MSG, and NTIME_MSG entries, only */
/* the types have to match.  If they do, return the address of */
/* internal buffer. */
			else if ( strcmp( entry->ut_line, BOOT_MSG ) == 0
			       || strcmp( entry->ut_line, OTIME_MSG ) == 0
			       || strcmp( entry->ut_line, NTIME_MSG ) == 0
				)
				{
				if ( strcmp( entry->ut_line, ubuf.ut_line ) == 0 )
					return &ubuf;
				}

			/* BRL -- no such thing as illegal non-null ut_line! */
			else if ( strncmp( entry->ut_line, ubuf.ut_line,
...
/* a pointer to it. */
		if ( strcmp( cur->ut_line, EMPTY ) != 0
		  && strcmp( cur->ut_line, BOOT_MSG ) != 0
		  && strcmp( cur->ut_line, OTIME_MSG ) != 0
		  && strcmp( cur->ut_line, NTIME_MSG ) != 0
		  && strncmp( entry->ut_line, cur->ut_line,
					sizeof cur->ut_line ) == 0
		   )	/* BRL -- rewritten for older UNIXes */
			return cur;
...
/* date in the future. */
	...
	/* BRL -- debug printout of ubuf.ut_id[*] removed */
	}
	fcntl(fd, F_SETFL, fc);
	return(answer);
}
...
char *newfile;
{
	extern char *strcpy();	/* DAG -- was missing */
	extern char utmpfile[];
