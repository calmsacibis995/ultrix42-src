#ifndef lint
static char *sccsid = "@(#)xdr_rec.c	4.1      ULTRIX  7/3/90";
#endif lint
	...
	int (*writeit)();  /* like write, but pass it a tcp_handle, not sock */
{
	register RECSTREAM *rstrm =
            (RECSTREAM *)mem_alloc(0, 0, sizeof(RECSTREAM), 0);
	...
	rstrm->the_buffer = (caddr_t) mem_alloc(0, 0, sendsize + recvsize + BYTES_PER_XDR_UNIT, 0);
	...
	register u_int s;
{
	...
	if (s < 100)
		s = 4000;
	else for (; (s % BYTES_PER_XDR_UNIT) != 0; ++s); /* round up */
	return (s);
}
