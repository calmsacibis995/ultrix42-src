 ...
 * 001- We no longer read yaccpar, which is the template parser, it is now
 *	in an include file called yaccpar.h.  The reason for this is
 *	so yacc can be part of the binary kit.
 *
 ***********************************************************************/
...
/*		SLR001
 *
 *	We no longer read the file yaccpar it is not part of an include
 *	file
 *
...
/*				SLR001
 *	This protion of the code has been rewritten so it now read the
 *	data structer called  yacclin which contains the template parser
 */
	for (  t = yacclin ; *t; t++ )
	    {
	    p = *t;
