#ifndef lint
static char sccsid[] = "@(#)n1.c	4.1	ULTRIX	7/17/90";
#endif lint

/*
 * Modification history:
 *
 * Mar 19, 88: Chetal Pradeep (chetal)
 *   	Undid the changes made (explained below) for backwards
 *	compatibility, historic reasons, and not to break some other
 *	programs.
 *
 * Sep 15, 87: Chetal Pradeep (chetal)
 * 	Code added for NROFF to recognize the TERM type even if the
 *	-T option is not given by the user, so as to pick up the proper
 *	terminal driving table.
 */
#include "tdef.h"
#include <sys/types.h>
#include <sys/stat.h>
