...
#ifndef lint
static	char	*sccsid = "@(#)sed1.c	4.1	(ULTRIX)	7/17/90";
#endif lint
...
*
*  	01	Lie-Min Hioe, 08-Jun-1989
*		test for NULL pointer to prevent core dump on PMAX
*		for certain regular expressions
*
*	02	Lie-Min Hioe, 21-July-1989
*		Make character class regular expression operate in an
*		8-bit transparent manner.		
************************************************************************/
...
char	*expbuf;
{
	...
	if(expbuf == NULL)			/* 01 - test for NULL pointer */
		return(0);			/*      to prevent core dump  */
...
/*fprintf(stderr, "*lp = %c, %o\n*ep = %c, %o\n", *lp, *lp, *ep, *ep);	/*DEBUG*/
	...
	case CCL|STAR:
		...
		do {
			c = *lp++ & 0377;
		} while(ep[(c & 0377) >>3] & bittab[c & 07]);		/* 02 */
		ep += 32;
		goto star;
