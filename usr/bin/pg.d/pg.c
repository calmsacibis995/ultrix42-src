#ifndef lint
static	char	*sccsid = "@(#)pg.c	4.2	(ULTRIX)	11/14/90";
#endif lint
...
/************************************************************************
*     Systems V static char sccsid[] = "@(#)pg.c	1.5";           *
************************************************************************/
...
*
*    22-Apr-88  David Gray 
*               Modified so the user responses are echoed.
*
*    20-Nov-88	Jon Reeves for Dave Long
*		Fixed missing arg on call to set_state().
*
*    15-Oct-90  GWS
*		changed #include <curses.h> to #include <cursesX.h> because
*		  this uses X/Open (System V-based) "curses".
*
************************************************************************/
...
#define USAGE() { fprintf(stderr,"Usage: pg [-number] [-p string] [-cefs] [+line] [+/pattern/] files\n"); exit(1); }
...
ttyin ()
{
	...
	fixterm();
	set_state(&state,' ',NULL);	/* initialize state processing */
	sptr = cmdbuf;
	while(state != 10) {
		ch = readch();
		if (ch == '\n' && !slash)
...
terminit()	/* set up terminal dependencies from termlib */
{
	...
	ioctl(fileno(stdout),TCGETA,&ntty);
 	...
 	/* ntty.c_lflag &= ~(ECHONL | ECHO | ICANON);  
           Changed to echo responses (gray) */
