 ...
 *
 *	9-Jan-89		Tim N
 *		Added usage error message for bad options.
 *		Added check for null pointer on command line when getting
 *		the -f switch.  Added sccs keywords, header and this comment
 *		block.  This module contained the ident:
 *				"@(#)main.c	4.2 8/11/83";
 ************************************************************************/
	...
	while (argc > 1) {
		...
		*/
		if (argv[0][0] == '-' && TOLOWER(argv[0][1]) == 'f' && argv[0][2] == '\0') {
			/* check for file name argument - Tim N */
			if (!argv[1])
			    error(FATAL, "Usage: awk [-f source | 'cmds'] [files]");
			yyin = fopen(argv[1], "r");
			if (yyin == NULL)
