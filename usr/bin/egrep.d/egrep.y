...
*
*  	01	Teoman Topcubasi, 30-dec-1987
*			changed static buffer allocation to dynamic
*			allocation.  No more RE too long!
*
*	02	Lie-Min Hioe, 1-May-89
*			8 bit cleaning, to provide 8 bit data transparency.
*
*	03	Lie-Min Hioe, 7-June-89
*			changed constant MAXPOS from 4000 to 8000
*			No more core dump on certain regular expressions 
*
...
#define MAXPOS 8000				/* 03 - used to be 4000 */
#define NCHARS 256				/* 02 - used to be 128 */
#define NSTATES 256				/* 02 - used to be 128 */
#define FINAL -1
unsigned char gotofn[NSTATES][NCHARS];		/* 02 - used to be char */
int state[NSTATES];
unsigned char out[NSTATES];			/* 02 - used to be char */
int line = 1;
int maxlin = 2048;
int *name;
int *left;
...
unsigned char *chars;				/* 02 - used to be char */
int nxtpos;
int nxtchar = 0;
int *tmpstat;
int *initstat;
...
int count;
int icount;
unsigned char *input;				/* 02 - used to be char */
FILE *exprfile;
...
int	f;
unsigned char	*fname;				/* 02 - used to be char */
%}
...
yylex() {
	extern int yylval;
	int cclcnt, x;
	register unsigned char c, d;		/* 02 - used to be char */
	switch(c = nextch()) {
		case '$':
		case '^': c = '\n';
			goto defchar;
...
nextch() {
	register int c;				/* 02 - used to be char */
	if (fflag) {
...
overflo() {
	/* Teoman Topcubasi, 12/28/87
	   changed the overflow handling to reallocate memory
	   rather than just issue an error message 
	 */
	maxlin = maxlin * 2;
	name = (int *)realloc(name, maxlin);
	left = (int *)realloc(left, maxlin);
	right = (int *)realloc(right, maxlin);
	parent = (int *)realloc(parent,maxlin);
	foll = (int *)realloc(foll, maxlin);
	chars = (unsigned char *)realloc(chars, maxlin);	/* 02 cast */
	tmpstat = (int *)realloc(tmpstat, maxlin);
	initstat = (int *)realloc(initstat, maxlin);
}
...
cgotofn() {
	register c, i, k;
	int n, s;
	unsigned char symbol[NCHARS];		/* 02 - used to be char */
	int j, nc, pc, pos;
	int curpos, num;
	int number, newpos;
	count = 0;
...
{
	/* Teoman Topcubasi, 12/28/87
	   added this routine to handle dynamic allocation of
	   buffers for egrep.
	 */

	name = (int *)malloc(maxlin);
	left = (int *)malloc(maxlin);
	right = (int *)malloc(maxlin);
	parent = (int *)malloc(maxlin);
	foll = (int *)malloc(maxlin);
	chars = (unsigned char *)malloc(maxlin);	/* 02 - unsigned cast */
	tmpstat = (int *)malloc(maxlin);
	initstat = (int *)malloc(maxlin);
}
...
out:
	if (argc<=0)
		exit(2);
	if (fflag) {
		fname = (unsigned char *) *argv;	/* 02 - cast */
	...
	else input = (unsigned char *)*argv;		/* 02 - cast */
	argc--;
	argv++;
...
char *file;
{
	register unsigned char *p;		/* 02 - used to be char */
	register cstat;
	register ccount;
	unsigned char buf[2*BUFSIZ];		/* 02 - used to be char */
