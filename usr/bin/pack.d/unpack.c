...
#ifndef	lint
static char sccsid[] = "@(#)unpack.c	4.1	ULTRIX	7/17/90";
#endif
	...
	for (k=1; k<argc; k++) {
		...
		*cp = '\0';
		...
		if (pcat)
			...
			if ((outfile = creat (argvk, (int) status.st_mode&07777)) == -1) {
				eprintf (": cannot create");
				goto done;
			}

			if (chmod (argvk, (int) status.st_mode) != 0)
			    VOID fprintf(stderr, "can't change mode to %o\n", status.st_mode);
...
expand ()
{
	...
	for (;;) {
		...
		if (Tree[tp] == 0) {
			...
			tp = 0;
			if ((origsize -= 1) == 0) {
				if (write (outfile, outbuff, outp - outbuff)
					== (outp - outbuff) )
					return(1);
				eprintf(": write error");
				return(0);
