...
#ifndef lint
static	char	*sccsid = "@(#)permiss.c	4.1	(ULTRIX)	7/17/90";
#endif lint
...
register struct packet *pkt;
{
	...
	if (*p == 'a'){
		...
		if (l_rel == pkt->p_gotsid.s_rel || l_rel == pkt->p_reqsid.s_rel) {
			locked++;
			sprintf(l_str,"release `%d' is get locked ",l_rel);
			break;
		}
	}
	if (locked) {
/* We are doing things that fatal would do but we are having to do it
   this way to allow make to continue on an sccs get SCCS when a file
   is locked for getting at the level we are getting   */
		Fflags = 0;
		Fvalue = 0;
		fprintf(stderr,"%s\n",l_str);
		clean_up(0);
