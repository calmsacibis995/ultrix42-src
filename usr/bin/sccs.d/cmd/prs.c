...
#ifndef lint
static	char	*sccsid = "@(#)prs.c	4.1	(ULTRIX)	7/17/90";
#endif lint
 ...
 ************************************************************************/
/*
 * 11/10/89 KJL - Avoid null pointer deference in read_mod.
 * 5/12/89 KJL - Changed to name of tzset to match definition in date_ab.c.
 * 1/25/88 DAL - Changed to allow mr lines with no mr data.
 */
/*************************************************************************/
/*									 */
	...
	for(lp = spec; *lp != 0; lp++) {
		if(lp[0] == ':' && lp[1] != 0 && lp[2] == ':') {
			...
			case 256*'K'+'L':	/* :LK: locked releases */
				...
				break;
			case 256*'G'+'L':	/* :LG: locked get releases */
				if (k = Sflags[LOCKGET - 'a'])
					printf("%s",k);
				else printf("none");
				break;
