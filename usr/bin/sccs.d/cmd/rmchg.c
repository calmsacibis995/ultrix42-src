...
#ifndef lint
static	char	*sccsid = "@(#)rmchg.c	4.1	(ULTRIX)	7/17/90";
#endif lint
 ...
 *	Also do error checking on fwrite's and fclose's.
 */
...
msg(app,name,cmrs,stats,sids,fred)
	...
	*ptr=(char)NULL;
	...
	if(fprintf(fd,"%s chpost %s q %s sw MID=%s MFS=%s q q\n",app,cmrs,pname,sids,stats)==EOF) {
		fatal("Error writing CASSI msg\n");
		return 0;
	}
	if(fclose(fd)==EOF) {
		fatal("Error writing CASSI msg\n");
...
char *p;
{
	...
	repl(mr_ptr,'\n','\0');
	mr_ptr += 3;
	for (argv = &Darg[PWVSTART]; *argv; argv++)
...
struct	packet	*pkt;
{
	...
	first = 1;
	for(argv = &Earg[PWVSTART]; *argv; argv++) {
		if (first) {
