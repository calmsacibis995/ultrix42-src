...
#ifndef lint
static	char	*sccsid = "@(#)admin.c	4.2	(ULTRIX)	12/6/90";
#endif lint
 ...
 *
 * 14 Sep 88 -- D. Long
 *	Use new LINEWIDTH constant for maximum text line length throughout
 *	instead of hard coded sizes and BUFSIZ.  Also check for input
 *	lines exceeding this size.
 *
 * 03 Mar 89 -- D. Long
 *	Applied fix for processing of LOCKFLAG and LOCKGET.
 */
...
char	*afile;
{
	...
	*/
	if (HADT) {
		if (*tfile) {
			iptr = xfopen(tfile,0);
			fgetchk(line, sizeof line,iptr,tfile,&gpkt);
			fclose(iptr);
		}
	...
	if (HADN) {		/*   N E W  F I L E   */
		...
		*/
		...
		if (HADI) {		/* get body */
			...
			*/
			stats.s_ins = fgetchk(line, sizeof line,iptr,ifile,&gpkt);
	...
	*/
	if (!HADH) {
		...
		xrename(auxf(gpkt.p_file,'x'),&gpkt);
...
register struct packet *pkt;
{
	...
	for (k = 1; fgets(strp,len,inptr); k++) {
		...
		i = strlen(strp);
		if(i == (len-1) && strp[i-1] != '\n') {
			sprintf(Error, "%s line %d is longer than %d characters", /* (ad??) */
				file, k, LINEWIDTH);
			fatal(Error);
		}
		if(strp[i-1] != '\n') {
			sprintf(Error, "%s line %d does not end with a newline", /* (ad??) */
				file, k);
			fatal(Error);
		}
