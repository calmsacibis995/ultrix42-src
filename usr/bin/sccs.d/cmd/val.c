...
register struct packet *pkt;
{
	...
	while (get_line(pkt) != NULL) {
		...
		if (*p++ != CTLCHAR)
			...
			/* The following code is commented out because it
			   was full of null pointers.  It should be fixed for
			   all hardware platforms.  Meanwhile it works fine
			   with the code added below...
				...
				addq(pkt,ser,iord == INS ? NO : NULL,iord,
							ap->a_reason & USER);
			... end of removed code, next line replaces it
			*/
				addq(pkt,ser,iord == INS ? NO:NULL,iord,USER);
		}
	}
	if (pkt->p_q)
		infile_err |= CORRUPT_ERR;
...
int iord;
{
	...
	/* added init of queue and added check for null pointer of cur */
	if (!pkt->p_q) {
	    pkt->p_q = q = (struct queue *) fmalloc(sizeof(*q));
	    cur = NULL;
	} else {
