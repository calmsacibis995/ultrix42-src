#ifndef lint
static char *sccsid = "@(#)diffdir.c	4.1	ULTRIX	7/17/90";
#endif lint
 ...
 *			previous version's sources.  Includes DL's
 *			eight bit work.
 *			This 4.3-5 source had id:
 *				"@(#)diffdir.c 4.11 (Berkeley) 10/22/87";
 ************************************************************************/
...
#ifdef mips
	if (cnt >= FILHSZ + AOUTHSZ) {
		AOUTHDR hdr;
		hdr = *(AOUTHDR *)&buf[FILHSZ];
		if (!N_BADMAG(hdr))
			return (0);
	}
#else
