	...
	struct proc *p;
{
	...
	pteaddr = &uptma[btokmx(p->p_stakbr)+p->p_stakpt-1];
	...
	pteaddr = uaddr(p); 
	readmem((char *)upt, (int)pteaddr, sizeof(upt));


	ncl = (UPAGES * NBPG + nbcl - 1) / nbcl;
	while (--ncl >= 0) {
		click = ncl * CLSIZE;
		physaddr = ctob(upt[click].pg_pfnum);
