...
#ifndef lint
static	char *sccsid = "@(#)xargs.c	4.1	(ULTRIX)	7/17/90";
#endif lint
/*
 *	28-July-1988, T. Topcubasi
 *	Original code adopted from System V Rel. 2
 *	I changed the call to wait which is different on BSD
 *	within the function lcall().
 *
 *	10-nov-88 rjl
 *		fixed null pointer bug in argument parsing loop
 *
 *	20-nov-88 reeves for dlong
 *		Added exit arguments
 *
 */
#include <stdio.h>
#include <strings.h>
...
char *sub, **subargs;
{
	...
	default:
		...
		if( iwait == -1  ||  retcode.w_stopval )
			return -1;
		return( retcode.w_retcode );
	case 0:
		execvp(sub,subargs);
		exit (1);
	case -1:
