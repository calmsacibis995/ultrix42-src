...
int j,k;
int g;
unsigned char *p;	/* 8 bit, changed to unsigned */
%}
acc	:	lexinput
	={	
# ifdef DEBUG
...
# endif
	...
	if(sect == DEFSECTION) {		/* definitions section */
		while(!eof) {
			if(prev == '\n'){		/* next signed char is at beginning of line */
				getl(p=buf);
				switch(*p){
				case '%':
					switch(c= *(p+1)){
...
/* To make certain that the output table size is correct when using 8 */
/*  bit chars. it is necessary to increase the user specified size by  */
/*  ZCHHALF. See  routine layout in sub2.c         wr 1989             */

					case 'o': case 'O':
						while (*p && !isdigit(*p))p++;
						outsize = siconv(p);
  /* increase size - 8 bit cleaning */		outsize = outsize + ZCHHALF;
						if (report ==2) report=1;
						continue;
