#ifndef lint
static CHTYPE *sccsid = "@(#)xec.c	4.1 (ULTRIX) 7/17/90";
#endif lint
 ...
 *
 * 004 - David Lindner Fri Nov  3 10:18:05 EST 1989
 *	 Changed calls from signal to nsignal, and externally
 *	 declared nsignal.
 *
 * 003 - Gary A. Gaudet - Wed May 10 15:46:19 EDT 1989
 *	 fixed a null pointer that caused SIGSEGV on ulimit for MIPS
 *
				...
				{		
					...
					else if (comtype == BUILTIN)
					{
						...
						case SYSCD:	
							...
							{	
								CHTYPE *safe;	/* DAG -- added (see note, below) */
								CHTYPE *cdpath;	
								CHTYPE *dir;	
								int f;	
 ...
 */
								safe = alloc(((unsigned)(length(cdpath) + length(a1))) /* GAG - to small */
									* (sizeof (CHTYPE)));  /* DAG -- added */
								do	
								{	
									dir = cdpath;	
								...
								while ((f = (chdir(wtoc(safe)) < 0)) && cdpath);	/* DAG */
								...
								{
									cwd(safe);	/* DAG -- added (see note, above) */
									if (cf(nullstr, dir) &&	
									    *dir != ':' &&	
									 	any('/', safe) &&	
									 	flags & prompt)	
		...
		case TFORK:
			...
			else	/* this is the forked branch (child) of execute */
			{
				...
				if (treeflgs & FINT)
				{
					nsignal(SIGINT, SIG_IGN);	/* DAG */
					nsignal(SIGQUIT, SIG_IGN);	/* DAG */
		...
		case TSW:
			{
				register CHTYPE	*r = mactrim(swptr(t)->swarg);
