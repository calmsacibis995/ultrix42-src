#ifndef lint
static  char    *sccsid = "@(#)sh.lex.c	4.2  (ULTRIX)        8/13/90";
#endif lint
...
/*
 * C shell
 * Modification History:	sh.lex.c
 *
 * 005 - Bob Fontaine	-  Fri Jun 22 09:53:01 EDT 1990
 *	 Changed call to internal printf function to csh_printf to avoid
 *	 confusion with stdio library routine.
 *
 * 004 - Gary A. Gaudet - Tue Jan  2 11:43:04 EST 1990
 *	changed register char *wp to register unsigned char
 *	changed char wbuf[] to unsigned char wbuf[]
 *	added some (castings)
 *	added return value check for ioctl()
 *
 * 03 12-Nov-88, Al Delorey (afd)
 *	ifdef the TENEX & CSHEDIT code.
 *
 * 02 20-Sep-88, Al Delorey (afd).
 *	Removed Mips tsh code (file completion & line edit).
 *	Added 'tenex' completion & dcsh style command line edit capability.
 *	Major mod to "bgetc()" for completion.
 *
...
lex(hp, didedit)
	...
	int didedit;			/* set true if we edited a cmd line */
#else
lex(hp)
	register struct wordent *hp;
#endif
{
	...
	if (! didedit) {
		...
		do
			c = readc(0);
		while (c == ' ' || c == '\t'); /* strip leading spaces and tabs */
		if (c == HISTSUB && intty)
	 ...
	 */
	do {
		register struct wordent *new = (struct wordent *) xalloc((unsigned)sizeof *wdp);
		...
		wdp = new;
#ifdef CSHEDIT
		wdp->word = (didedit ? editword() : word());
#else
		wdp->word = word();
#endif
	} while (wdp->word[0] != '\n');
	...
	struct wordent *sp0;
{
	...
	for (;;) {
		stripstr = savestr(sp->word);
		csh_printf("%s", strip(stripstr)); 	/* 005 RNF */
		xfree(stripstr);
		sp = sp->next;
		if (sp == sp0)
			break;
...
word()
{
	register CHTYPE c, c1;
	register unsigned char *wp;	/* 004 - GAG */
	unsigned char wbuf[BUFSIZ*2];	/* 004 - GAG */
...
/*
 *			alnum(c) is a macro
 *			getC(DOEXCL) will execute twice
 *			so don't use alnum(c = getC(DOEXCL)
 */
			c = getC(DOEXCL);
			while (alnum(c)) {
				if (np < &name[sizeof name / 2])
...
#ifdef CSHEDIT
	case 'v':			/* modifier for line edit - afd */
		editline++;
		goto ret;
#endif
...
csh_printf("Reset tty pgrp from %d to %d\n", ctpgrp, tpgrp); /* 005 RNF */
					goto reread;
				}
				if (adrof("ignoreeof")) {
					if (loginsh)
