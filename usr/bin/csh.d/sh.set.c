#ifndef lint
static  char    *sccsid = "@(#)sh.set.c	4.2  (ULTRIX)        8/13/90";
#endif lint
 ...
 *
 * 004 - Bob Fontaine   - Thu May 24 14:07:57 EDT 1990
 * 	 If the user sets $home to a path that does not begin with a '/',
 *	 inform him/her that fact and warn that future actions may be 
 *	 unpredictable.	 Fix for QAR #2622
 *
 * 003 - Gary A. Gaudet - Tue Jan  2 12:01:30 EST 1990
 *	added limits.h
 *	cleanup some INT_MIN code
 *	added some (castings)
 *
 * 02 20-Sep-88 Al Delorey (afd)
 *	removed some old tcsh code (using dcsh form now).
 *
...
setsyn:
		...
		if (eq(p, "(")) {
			...
			*e = p;
			...
			set(vp, savestr(p));
			if (eq(vp, "home") && *p != '/')	/* 004 RNF */
	oops("Warning: home is not set to an absolute path name");
			}
		if (eq(vp, "path")) {
...
set(var, val)
	char *var, *val;
{
	register char **vec = (char **) xalloc((unsigned)(2 * sizeof (char **)));
	...
	register struct varent *p;
{
	...
	p->v_link[f] = c = (struct varent *)xalloc((unsigned)sizeof (struct varent));
	...
	if (val)
		while (*val) {
			if (strlen(*val) + strlen(exppath) + 2 > BUFSIZ) {
				csh_printf("Warning: ridiculously long PATH truncated\n"); /* 005 RNF */
				break;
			}
