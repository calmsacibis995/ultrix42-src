#ifndef lint
static  char    *sccsid = "@(#)sh.c	4.2  (ULTRIX)        8/13/90";
#endif lint
/************************************************************************
 *									*
 *			Copyright (c) 1988 by				*
 ...
 *
 * 005 - Bob Fontaine - Thu Jun 21 10:22:20 EDT 1990
 *	change name of csh internal printf function to csh_printf to avoid
 *	conflicts with printf function in stdio library.
 *
 * 004 - Bob Fontaine - Fri May 25 10:00:14 EDT 1990
 *	Stat argument to source command and make sure file exists and
 *	that it is a regular file (ie that it is not a directory, pipe ...).
 *	Fix for QAR# 3835
 *
 * 003 - Gary A. Gaudet - Thu Dec 28 17:31:15 EST 1989
 *	Added comments; removed code not compiled.
 *
 * 02	12-Nov-88, Al Delorey (afd).
 *	Use "ifdef CSHEDIT" around the cmd line edit code.
 *
 * 01	20-Sep-88, Al Delorey (afd)
 *	Added command line edit capability (search for "edit" to find changes).
 *	Deleted Mips tsh_init call.
 */
...
/*
 *	immediately establish a timing base
 */
	settimes();
	v = av;
/*
 ...
 */
	uid = getuid();
/*
 *	login shell if "-csh" with no arguments
 */
	loginsh = **v == '-' && c == 1;
	if (loginsh)
		(void) time(&chktim);
...
notty:
  				csh_printf("Warning: no access to tty; thus no job control in this shell...\n"); /* 005 RNF */
				tpgrp = -1;
			}
		}
	}
