#ifndef lint
static	char *sccsid = "@(#)pr.c	4.1	(ULTRIX)	7/17/90";
#endif lint
 ...
 *
 * 11-Sep-88    David Gray (gray)
 *              Made sure "move" always had a value, problem occurred
 *              when tabs were encountered and neither the -i or -e
 *              option were specified, therefore set move = 8, DJG#3
 *              Problem exposed due to previous fix.
 *
 * 16-Aug-88    Dave Gray (gray)
 *              1) Added "continue" statement after 'n' & 'x' options,
 *                 prevents the automatic inclusion of the 'b' option.
 *              2) Fixed -w option, previously had no effect on output.
 *
 *              Fixes labeled DJG#1 and DJG#2, respectively
 *
 *
 * 08-Jun-88	Mark Parenti (map)
 *
 *		Changed signal handlers to void.
 *
 * 11-Feb-88    David Gray (gray)
 *
 *              added -b option which prints blank headers
 *
 *******************************************************************/
...
int Dblspace = 1, Fpage = 1, Formfeed = 0,
	...
	Etabn = 0, Etabc = '\t', Itabn = 0, Itabc = '\t', nhdr = 0;
char *Head = NULL;
