...
#define	TMPLEN	19	/* size of temp file names */
...
char	*xref,	/* path to the cross-referencer */
	...
	*ofile;	/* pending output file name */
...
char	xflnm[MAXWORD],	/* saves current filename */
	...
	sv3[MAXRFL],
	sv4[MAXRFL],
	tmp1[TMPLEN],	/* contains preprocessor result */
	tmp2[TMPLEN],	/* contains xcpp and xpass cross-reference */
	tmp3[TMPLEN],	/* contains transformed tmp2 */
	tmp4[TMPLEN];	/* contains sorted output */
...
mktmpfl()
{
	...
	strncpy(tmp1, "/usr/tmp/xr1XXXXXX", TMPLEN);
	strncpy(tmp2, "/usr/tmp/xr2XXXXXX", TMPLEN);
	strncpy(tmp3, "/usr/tmp/xr3XXXXXX", TMPLEN);
	strncpy(tmp4, "/usr/tmp/xr4XXXXXX", TMPLEN);
	mktemp(tmp1);
	mktemp(tmp2);	/* holds output of "xpass" */
	mktemp(tmp3);	/* holds output of tmpscan() routine */
	mktemp(tmp4);	/* holds output of tempfile 3 */
