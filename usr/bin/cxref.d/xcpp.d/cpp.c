#ifndef lint
static	char	*sccsid = "@(#)cpp.c	4.1	(ULTRIX)	7/17/90";
#endif lint
 ...
 *
 * 30-Sep-88  D. Long/J. Reeves
 *	hacked a call to ppwarn to pass required number of args.
 *	Assorted other varargs cleanup.
 */
...
*/
	...
	if (bob(np+1)) {pperror("token too long",0); np=pbeg; p=inp+BUFSIZ;}
...
*/
	...
	if (fretop>0) np=bufstack[--fretop];
		...
		if (savch>=sbf+SBSIZE) {pperror("no space",0); exit(exfail);}
		*savch++='\0';
	}
	...
	if (bob(np)) pperror("token too long",0);
...
# endif
	...
	if((nfil=savch)>sbf+SBSIZE-BUFSIZ) {pperror("no space",0); exit(exfail);}
	filok=0;
