#ifndef lint
static	char	*sccsid = "@(#)fio.c	4.1	(ULTRIX) 7/2/90";
#endif lint
...
acct:
	...
	sprintf(ibuf, ret == SUCCESS ?
		...
		"send failed after %ld bytes %ld.%02d secs",
		fbytes, (long)t2.time, mil / 10);
	sysacct(abytes, (time_t)(t2.time + (mil >= 250 ? 1 : 0)));
	if (retries > 0)
...
acct:
	...
	sprintf(ibuf, ret == SUCCESS ?
		...
		"receive failed after %ld bytes %ld.%02d secs",
		fbytes, (long)t2.time, mil / 10);
	sysacct(abytes, (time_t)(t2.time + (mil >= 250 ? 1 : 0)));
	DEBUG(1, "%s\n", ibuf);
	syslog(ibuf);
