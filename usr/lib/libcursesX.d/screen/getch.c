#ifdef lint
static char *sccsid = "3/1/91	(ULTRIX)	@(#)getch.c	4.2";
#endif lint
...
/*
 *	30-May-89 Andy Roach
 *      --------------------
 *      Fixed select timeval structure. Cursor keys now return KEY_*
 *
 */
int
wgetch(win)
 ...
 * 
 * The AT&T code had the last param to the select as 100, this
 * should was fixed to have a timeval structure.
 */
static
_fpk(f)
FILE *f;
{
	...
	outfd = exfd = (fd_set *) NULL;
	rc = select(_curses_dtablesize(), &infd, outfd, exfd, &timeout);
	if (rc < 0)
		return -2;
	rc = read(fileno(f), &c, 1);
	return rc == 1 ? c : -1;
