 ...
 *
 * 02/20/90 GWS	removed code which reset 'c' to same value as 'rawc', because
 *		 old code caused passed character attributes to be stripped.
 */
...
# endif
	...
	case '\t':
	  ...
	  default:
		...
		{
			uctr = unctrl(rawc);
			waddch(win, (chtype)uctr[0]|(c & 0x7F));
			waddch(win, (chtype)uctr[1]|(c & 0x7F));
			return OK;
		}
		if( win->_attrs )
		{
