#ifndef lint
static char *sccsid = "@(#)clreolinln.c	4.1	(ULTRIX)	7/2/90";
#endif lint
 ...
 *
 * 02/28/90 GWS	in _inslines(): replaced "*insert_line==0" NULL pointer
 *		 dereferences with "insert_line==NULL || *insert_line==NULL"
...
_inslines (n)
{
	...
	} else if (parm_insert_line && (n>1 || (insert_line==NULL || *insert_line==NULL))) {
		tputs(tparm(parm_insert_line, n, SP->phys_y), lines-SP->phys_y, _outch);
	} else if (change_scroll_region && (insert_line==NULL || *insert_line==NULL)) {
