...
#ifndef lint
static	char	*sccsid = "@(#)lg02of.c	4.1   (ULTRIX)	7/2/90";
#endif lint
 ...
 * lg02of.c	
 *
 * LG02 LINE PRINTER FILTER
 *
 * 23-Nov-88 - Dave Gray (gray)
 *
 *      Fixed bad pointer reference in strncmp
 *
 * This filter is essentially the ln03of filter with some
 * modifications.
 */
...
#define PLD	  '\113'	/* upper case K = lg02of partial line down */
#define PLU	  '\114'	/* upper case L = lg02of partial line up */
#define FLU	  '\115'	/* upper case M = lg02of full line up */
...
char	*setlocale();				/* internationalization function */
...
determinefile()
{
	...
	if(filestorage[0] == '\033' && filestorage[1] == 'c')
		return(ANSI_FILE);
...
/* If the file stream is of the XIMAGE_FILE type this routine is called
   from lg02of.  The routine creates a sixel output stream to send
