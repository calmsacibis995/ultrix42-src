#ifndef lint
static	char	*sccsid = "@(#)deliver.c	4.1	(ULTRIX)	7/2/90";
#endif lint
...
# endif DEBUG
	...
	*/
	...
	if (index(host, '.') != 0)	/* canonical if has dots */
		_res.options &= ~(RES_DEFNAMES | RES_DNSRCH);
	else				/* not canonical 'cause local */
...
# ifdef SMTP
	...
	{
		register int i;
		register struct svcinfo *svcinfo;
		...
		if ((svcinfo = getsvc()) != NULL)
			for (i = 0; (svc_lastlookup = svcinfo->svcpath[SVC_HOSTS][i++]) != SVC_LAST; )
				if (svc_lastlookup == SVC_BIND)
				{
					if ((nmx = getmxrr(host, mxhosts, buf, &rcode)) < 0) {
						if (rcode == EX_NOHOST) {
							/*
							 * ignore not having a host in BIND
							 * as we may later find it elsewhere
							 */
#ifdef DEBUG
							printf("getmxrr: EX_NOHOST\n");
#endif DEBUG
							rcode = EX_OK;
							nmx = 1;
							mxhosts[0] = host;
...
#ifdef DEBUG
						printf("getmxrr() found the following hosts:\n");
						for (i = 0; i < nmx; i++)
							printf("   %s\n", mxhosts[i]);
#endif
						/* Just done successful MX lookup */
						break;
					}
					/* break out of loop now have called getmxrr() */
					break;
				}
		...
		if (rcode == EX_OK)
		{
			message(Arpa_Info, "Connecting to %s (%s)...", mxhosts[0], m->m_name);
#ifndef DNUMODS
			rcode = smtpinit(m, pv);
#else DNUMODS
			rcode = smtpinit(m, pv, e);
#endif DNUMODS
		}
...
#ifdef DNUMODS
				if( rcode == EX_OK ) {
				    extern int SmtpMultiStatus;

				    if( ! SmtpMultiStatus )
					rcode = smtpdeliverstat(m);
				    else
					for (tobuf[0] = '\0', to = tochain; to != NULL; to = to->q_tchain)
					{
					    int i;
...
# ifdef LOG
	...
	/*
	 * imcd/jch - fix to long log lines
	 *
	 * The syslog V2.4 facility is brain dead.  It only handles
	 * 5 arguments to sprintf!  It core dumps if the message is
	 * longer that 1000 bytes...  The daemon corrupts its time counter...
	 *
	 * Thus this code:
	 */
	delay = pintvl(curtime() - CurEnv->e_ctime, TRUE);
	len=strlen(CurEnv->e_to);
	if (len <= LOG_MAXTO)
		return(syslog(LOG_INFO, "%s: to=%s, delay=%s, stat=%s",
			CurEnv->e_id, CurEnv->e_to, delay, stat));

	for (tp=CurEnv->e_to; len>0; len -= LOG_MAXTO, tp += LOG_MAXTO)
		syslog(LOG_INFO, "%s: to=%s%.*s", CurEnv->e_id,
			(tp==CurEnv->e_to)?"":"(cont) ", LOG_MAXTO, tp);

	syslog(LOG_INFO, "%s: delay=%s, stat=%s", CurEnv->e_id, delay, stat);
# endif LOG
}
/*
