 ...
 * EDIT HISTORY
 *
 * 001	01-April-1988	Jeff Michaud
 *	For the "HEAD" command, dump out the message body in addition to the
 *	message headers.  This makes the "HEAD" command basically look like
 *	a "DATA" command.
 */
...
# ifndef SMTP
# ifndef lint
static char	SccsId[] = "@(#)usersmtp.c	4.1	(ULTRIX) (no SMTP)	7/2/90";
# endif not lint
# else SMTP

# ifndef lint
static char	SccsId[] = "@(#)usersmtp.c	4.1	(ULTRIX) (with SMTP)	7/2/90";
# endif not lint
...
#ifdef DNUMODS
int	SmtpMultiStatus;		/* mailer will give reply for each
					   recipient after "DATA" done	   */
#endif DNUMODS
...
#ifdef DNUMODS
	/*
	**  Ask mailer if it will be returning multi-status for DATA command
	*/
	if( bitnset(M_XDEVSTATUS, m->m_flags) )
	{
		...
		SmtpMultiStatus = (r == 250);
	}
	else
		SmtpMultiStatus = FALSE;

	/*
	**  If this mailer wants the envelope header early, see if
	**  he will accept it.
	*/
...
# endif LOG
		...
		if (SmtpReplyBuffer[0] == '4' && strlen(SmtpReplyBuffer) > 4)
