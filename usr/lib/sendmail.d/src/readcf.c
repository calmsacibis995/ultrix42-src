#ifndef lint
static	char	*sccsid = "@(#)readcf.c	4.1	(ULTRIX)	7/2/90";
#endif lint
...
/* In support of Yellow Pages, we need to use the sscanf logic. Define
 * the flag. Why is this set up this way I wonder ?
 */
# define SCANF 1
...
# ifdef SCANF
		...
		/* /etc/passwd file entries for Yellow Pages users
		 * of the form  +name:::::/home/dir:
		 * must have the  +  stripped, else the user name
		 * won't get into the class.
		 */
		if (buf[0] == '+') {
			if (sscanf(&buf[1], fmt, wordbuf) != 1)
				continue;
		}
		else {
