...
# endif V6
	...
	/* Get Yellow Pages domain (if any)
	 */
	if (!getdomainname(default_domain,YPMAXDOMAIN) && 
		   (strlen(default_domain) > 0)) 

			YPdomain = default_domain;
		else
			/* If we can't get a domain name for YP,
			 * assume YP not up and therefor, no YP maps.
			 */
			YPdomain = NULL;

#ifdef LOG
# ifndef BSD4_3
	openlog("sendmail", LOG_PID|LOG_CONS|LOG_NDELAY, LOG_DAEMON);
# else	BSD4_3
	openlog("sendmail",LOG_PID|LOG_TIME);
# endif BSD4_3
#endif LOG
	errno = 0;
	from = NULL;
	...
	if (readconfig)
	{
		...
		{
		int i=0;
			/* Strip trailing domain spec so that $w macro
			 * is unqualified host name for config file
			 * rules parsing.
			 */
                        while (jbuf[i] != '\0' && jbuf[i] != '.')
                        {
                                i++;
                        }
                        jbuf[i] = '\0';
...
# ifdef LOG
	if (LogLevel > 11)
		syslog(LOG_INFO, "finis, pid=%d", getpid());
# endif LOG
...
# ifdef LOG
	if (LogLevel > 11)
		syslog(LOG_INFO, "in background, pid=%d", getpid());
# endif LOG
