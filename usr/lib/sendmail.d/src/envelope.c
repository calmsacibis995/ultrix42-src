#ifndef lint
static	char	*sccsid = "@(#)envelope.c	4.1	(ULTRIX)	7/2/90";
#endif lint
...
#ifdef LOG
	if (LogLevel > 10)
		syslog(LOG_INFO, "dropenvelope, id=%s, flags=%o, pid=%d",
...
# ifdef LOG
	if (LogLevel > 19)
		syslog(LOG_INFO, "%s: openx%s", e->e_id, e->e_xfp == NULL ? "" : " (no)");
# endif LOG
	if (e->e_xfp != NULL)
		return;
	p = queuename(e, 'x');
