...
# ifndef lint
# ifndef DAEMON
static char	SccsId[] = "@(#)daemon.c	4.2	ULTRIX	8/9/90 (w/o daemon mode)";
# else
static char	SccsId[] = "@(#)daemon.c	4.2	ULTRIX	8/9/90 (with daemon mode)";
# endif
# endif /* not lint */
...
# ifdef LOG
			if (LogLevel > 11)
				syslog(LOG_INFO, "connected, pid=%d", getpid());
# endif LOG
			return;
		}
...
extern int	h_errno;	/*this will go away when code implemented*/
	...
	FILE **infile;
{
	...
	extern struct hostent *gethostbyname_bind();
	register struct svcinfo *svcinfo;
	register int i, s, svc;
	extern char *inet_ntoa();
	int sav_errno;
	...
	*/
	...
	else
	{
		if ((svcinfo = getsvc()) != NULL)
			for (i = 0; (svc = svcinfo->svcpath[SVC_HOSTS][i++]) != SVC_LAST; )
				if (svc == SVC_BIND)
				{
					hp = gethostbyname_bind(host);
					break;
				}
		if (hp == NULL)
	...
	int hbsize;
{
	...
	extern struct hostent *gethostbyname_bind();
	extern struct hostent *gethostbyaddr_bind();
	register struct svcinfo *svcinfo;
	register int i, svc;
		...
		*bptr = '\0';
		...
		if ((svcinfo = getsvc()) != NULL)
			for (i = 0; (svc = svcinfo->svcpath[SVC_HOSTS][i++]) != SVC_LAST; )
				if (svc == SVC_BIND)
				{
					hp = gethostbyaddr_bind((char *) &in_addr, sizeof(struct in_addr), AF_INET);
					break;
				}
		if (hp == NULL)
