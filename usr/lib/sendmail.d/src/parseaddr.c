#ifndef lint
static	char	*sccsid = "@(#)parseaddr.c	4.1	(ULTRIX)	7/2/90";
#endif lint
 ...
 * EDIT HISTORY
 *
 * 001	07-July-1988	Jeff Michaud in association w/John Haxby
 *	The recipient names in RCPT commands we were issuing were
 *	getting the domain of the sender tacked on if they didn't
 *	already have a domain and the 'C' flag for the incomming
 *	mailer was set.  The 'C' flag is documented to only affect
 *	the rewriting of the addresses in header lines, not in the
 *	envelope.
 *
 ***********************************************************************/
	...
	register char c;
{
	...
	if ( c == MATCHMA || c == MATCHPW || c == MATCHCLASS || c == MATCHREPL || c == MATCHNCLASS)
		return (ONE);
	if (c == '"')
		return (QST);
	if (!isascii(c))
...
# endif DEBUG
			...
			switch (*rp)
			{
			  ...
			  case MATCHMA:
				/* See if token is in the name server
				 * db for aliases.
				 */
				if (!in_alias_map(rvp,ap))
					goto backup; /*_not found_*/
				mlp->first = avp;
				mlp->last = avp++;
				mlp++;
				break;
	...
	register ADDRESS *b;
{
	...
	/**
	 ** For Reading mail+ support..  (mailplus)
	 **
	 ** If the mailer is 'prog'  AND  the addressess from which we are
	 ** aliased are owned by different  UIDs, then return false.
	 ** Different programs are picked up later. The paranoia checks
	 ** are probably not needed.  Quote: JCH  :Enquote
	 **/
	if (a->q_mailer == ProgMailer &&
		a->q_alias != NULL &&
		b->q_alias != NULL &&
		a->q_alias->q_flags|QGOODUID &&
		b->q_alias->q_flags|QGOODUID &&
		a->q_alias->q_uid != b->q_alias->q_uid)
			return FALSE;
...
# endif DEBUG
	...
	*/
	...
	if (pvp == NULL)
		return (name);
	rewrite(pvp, 3);
	if( ! canonical  &&  CurEnv->e_fromdomain != NULL ) /* 001 */
	{
...
/*
**
**  IN_PASSWD_MAP -- See if something is in a passwd map
**
**	Parameters:
**		rvp	-- rewrite rule vector pointer
**		ap	-- argument vector
**			rather than the recipient rewriting rules.
**
**	Returns:
	...
	char *name;
{
	...
	if ((svcinfo = getsvc()) != NULL)
		for (i=0; (svc_lastlookup = svcinfo->svcpath[SVC_PASSWD][i]) != SVC_LAST; i++)
			if (p = ((*(getpasswdnames [svcinfo->svcpath[SVC_PASSWD][i]])) (name) ))
				break;
	return(p);
}
...
in_passwd_map(rvp,ap)
	char **rvp;	
	char **ap;
{
	int err;

	if ((getpasswdname(ap)) == NULL) {
		/*
		 * Assume not in the map.
		 */
# ifdef DEBUG
		if (tTd(21,1))
			printf("Map entry  NOT found for %s\n",ap);
# endif DEBUG
		rvp--;
		return(NULL);
	}
...
/*
**
**  IN_ALIAS_MAP -- See if something is in an aliases map
**
**	Parameters:
**		rvp	-- rewrite rule vector pointer
**		ap	-- argument vector
	...
	char *name;
{
	...
	if ((svcinfo = getsvc()) != NULL)
		for (i=0; (svc_lastlookup = svcinfo->svcpath[SVC_ALIASES][i]) != SVC_LAST; i++)
			if (p = ((*(getaliasnames [svcinfo->svcpath[SVC_ALIASES][i]])) (name) ))
				break;
	return(p);
}
