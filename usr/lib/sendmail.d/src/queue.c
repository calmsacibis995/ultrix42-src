...
# ifdef LOG
		if (LogLevel > 11)
			syslog(LOG_INFO, "%s: dowork, pid=%d", CurEnv->e_id,
			       getpid());
# endif LOG
...
# ifdef LOG
			if (LogLevel > 4)
				syslog(LOG_INFO, "%s: locked", CurEnv->e_id);
# endif LOG
			if (ForkQueueRuns)
				exit(EX_OK);
			else
...
# endif DEBUG
		switch (buf[0])
		{
		  case 'R':		/* specify recipient */
			sendtolist(setctluser(&buf[1]), (ADDRESS *) NULL, &e->e_sendqueue);
			clrctluser();
			break;

		  case 'E':		/* specify error recipient */
			sendtolist(setctluser(&buf[1]), (ADDRESS *) NULL, &e->e_errorqueue);
			clrctluser();
			break;
...
# ifdef LOG
		if (LogLevel > 16)
			syslog(LOG_INFO, "%s: assigned id", e->e_id);
# endif LOG
# endif DEBUG
	}
...
# ifdef LOG
	if (LogLevel > 19)
		syslog(LOG_INFO, "%s: unlock", e->e_id);
# endif LOG
	if (!tTd(51, 4))
#endif DEBUG
...
/*
**  BADCTLADDR - ensure that an address does not look like "(user) xxx". EOL
