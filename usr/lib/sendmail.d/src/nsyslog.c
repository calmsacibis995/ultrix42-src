#ifndef lint
static	char	*sccsid = "@(#)nsyslog.c	4.1	(ULTRIX)	7/2/90";
#endif lint
	...
	char *fmt;
{
	...
	while (*f != '\0')
	{
			...
			switch (c)
			{
			  case 'm':	/* output error code */
				if (errno < 0 || errno > sys_nerr)
					(void) sprintf(b, "error %d", errno);
				else
					(void) sprintf(b, "%s", sys_errlist[errno]);
				break;
		...
		*b = '\0';
		...
		/* output string */
		(void) sprintf(outline, buf, p0, p1, p2, p3, p4);
#ifdef LOG_IPC
