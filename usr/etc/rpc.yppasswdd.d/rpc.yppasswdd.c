#ifndef lint
static char *sccsid = "@(#)rpc.yppasswdd.c	4.1      ULTRIX  7/2/90";
#endif lint
 ...
 *	--------
 *
 * Oct 31, 1989 dal/ Fixed problem handling errors during passwd update
 *
 * Apr 20, 1989 jhw/ Fixed security hole
 *
 * Jun 12, 1988 jhw/ Fixed username parsing in getpwnam
 *
 * Jun 09, 1988 map/ Changed signal handlers to void.
 *
 ...
 */
	strcpy(temp, file);
	if(!strcmp(temp, "/etc/passwd"))
		strcpy(temp, "/etc/ptmp");
	else
		strcat(temp, ".tmp");
/*
 * Lock the map source file.
 */
	...
	/* make sure that original file is secure */
	chmod(file, 0644);
...
register char *s1, *s2;
{
        while (*s1 == *s2)            /* bump while they match */
              {
              *s1++;
              *s2++;
              }
        if(*s1 == '\0' && *s2 == ':')  /* format specific solution */                         return(0);
	return(1);
} 
	...
	char *name;
{
	...
	if (pwf == NULL)
		return (NULL);
	cnt = strlen(name);
	while ((p = fgets(line, BUFSIZ, pwf)) && pwcmp(name, line))
		continue;
	if (p) {
		passwd.pw_name = p;
		p = pwskip(p);
