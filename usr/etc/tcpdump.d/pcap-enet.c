 ...
 * subroutines.
 *
 * SCCSID: @(#)pcap-enet.c	4.1	ULTRIX	1/25/91
 *
	...
	int fd;
{
	...
	printf("%d packets queued", es.enStat_Rcnt);
	if (es.enStat_Rdrops > 0)
		printf(", %d dropped", es.enStat_Rdrops);
	if (es.enStat_Reads > 0)
		printf(", %d tcpdump %s", es.enStat_Reads,
				es.enStat_Reads > 1 ? "reads" : "read");
	if (es.enStat_MaxRead > 1)
		printf(", %d packets in largest read", es.enStat_MaxRead);
	printf("\n");
#endif	/* IBMRTPC */
	close(fd);
}
