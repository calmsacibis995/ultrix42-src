#ifndef lint
static char *sccsid = "@(#)showmount.c	4.2      ULTRIX  2/21/91";
#endif lint
	...
	char **argv;
{
	...
	if (hp = gethostbyname(host))
		...
		fprintf(stderr,"showmount: gethostbyname of %s failed\n", host);
		exit(1);
	}
	...
	if (cl == NULL) {
		(void)close(s);
		s = RPC_ANYSOCK;
		cl = clntudp_create(&sin, MOUNTPROG, MOUNTVERS, TIMEOUT, &s);
		if (cl == NULL) {
	...
	if (err = clnt_call(cl, MOUNTPROC_DUMP, xdr_void, 0, xdr_mountlist, &ml, TIMEOUT)) {
