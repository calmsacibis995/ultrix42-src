#ifndef lint
static	char	*sccsid = "@(#)rlogind.c	4.1	(ULTRIX)	7/2/90";
#endif lint
 ...
 *
 * 09-Sep-89 lp
 *	Fix for wtmp cleanup (add locking mechanism)
 *
 * 09-Jun-88 map
 ...
 *
 * 18-May-87 lp
 *	Bump pty search from p-s to p-z (giving more pty's).
 *
...
#define TTY_DEBUG	/* report to syslog if tty not found */
	...
	struct sockaddr_in *fromp;
{
	...
	if (hp == 0) {
	...
	else if (svc_lastlookup == SVC_BIND) {
		/*
		...
		if (hp)
			for (; hp->h_addr_list[0]; hp->h_addr_list++)
				if (!bcmp(hp->h_addr_list[0], (caddr_t)&fromp->sin_addr, sizeof(fromp->sin_addr))) {
					hostok++;
					break;
				}
	}
	...
	write(f, "", 1);
	if (hostok == 0)
		fatal(f, "Host address mismatch.  Permission denied");
	for (c = 'p'; c <= 'z'; c++) {
		struct stat stb;
		line = "/dev/ptyXX";
...
#ifdef TTY_DEBUG
if(!found)
syslog(LOG_ERR, "notfound %10s %d", line, nutmp);
#endif TTY_DEBUG
		}
		close(f);
	}
