/* SCCSID: @(#)bootparam_subr.c	4.1	7/2/90 */
	...
	struct in_addr client_addr;
{
	...
	/* JSD: only attempt route lookup if '-D' (2) specified, since
	 * lookups are currently broken.  Just return client addr...
	 */
	if (debug != 2)
		return(client_addr);
	...
	for (i = 0; i < RTHASHSIZ; i++) {
		...
		for (m = routehash[i]; m; m = mb.m_next) {
			...
			if (read(kmem, (char *) &mb, sizeof (mb)) != sizeof (mb)) {
				printf ("read routehash from kmem failed.\n");
				exit (1);
			}
