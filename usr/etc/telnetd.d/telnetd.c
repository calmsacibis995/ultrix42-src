#ifndef lint
static	char	*sccsid = "@(#)telnetd.c	4.1	(ULTRIX)	7/2/90";
#endif lint
 ...
 *
 * 30-Aug-88    Michael G. Mc Menemy
 *      Fixed hang in cleanup by closing master side of pty before
 *      calling vhangup.
 *      Flush any remaining output from slave pty to the master pty for
 *      the telnet client.
 *
...
#else
	{
		char buffer[TABBUFSIZ], barea[TABBUFSIZ];
		char *getstr(), *area = barea;
		if(getent(buffer, "default") >0) {
			BANNER1 = getstr("im", &area);
			bcopy(BANNER1, nfrontp, strlen(BANNER1));
			nfrontp += strlen(BANNER1);
		}
	}
#endif
...
cleanup()
{
  ...
  for (;;) {
    fd_set ibits, obits, xbits;
    register int c;
    static struct timeval tmp_time = {0,0};
    ...
    if ((c = select(16, &ibits, &obits, &xbits, &tmp_time)) < 1) {
      break;
    }
	...
	char **area;
{
	...
	while ((c = *str++) && c != ':') {
		...
		case '^':
			c = *str++ & 037;
			break;
	/* Hack to parse the hostname field %h %@#$@$ */
		case '%':
			*cp++ = c;
			c = *str++;
			if(c == 'h') {
				cp -= 1;
				bcopy(hostname, cp, strlen(hostname));
				cp += strlen(hostname);
				continue;
			}
			break;
