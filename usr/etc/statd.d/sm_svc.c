...
# ifndef lint
static char *sccsid = "@(#)sm_svc.c	4.1	(ULTRIX)	7/2/90";
# endif not lint
 ...
 *
 *	 3-Feb-88	fries
 *			Removed commenting to allow parent to exit
 *			if debug == 0 and nofork == 0.
 *
 *	18-Jan-88	fries
...
/*!!!*/		case 'f':
			...
			break;
		default:
			fprintf(stderr, "in.statd -d[debug] -D\n");
			return;
		}
	if(choice == 0) {
...
/*!!!*/	if(!debug && !nofork) {
		ppid = fork();
		if(ppid == -1) {
			(void) fprintf(stderr, "in.lockd: fork failure\n");
			(void) fflush(stderr);
			abort();
		}
		if(ppid != 0) {
