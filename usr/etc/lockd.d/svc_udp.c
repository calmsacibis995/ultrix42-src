...
# ifndef lint
static char *sccsid = "@(#)svc_udp.c	4.1	(ULTRIX)	7/2/90";
# endif not lint
 ...
 *	25-Jan-88	fries
 *			Replaced calls to mem_alloc() with malloc().
 *
 *	18-Jan-88	fries
	...
	u_int sendsz, recvsz;
{
	...
	xprt = (SVCXPRT *)malloc(sizeof(SVCXPRT)); /* Ultrix mod fsg */
	if (xprt == NULL) {
		fprintf(stderr, "svcudp_create: out of memory\n");
		return (NULL);
	}
	su = (struct svcudp_data *)malloc(sizeof(*su)); /* Ultrix mod fsg */
	if (su == NULL) {
		fprintf(stderr, "svcudp_create: out of memory\n");
		return (NULL);
	}
...
#define ALLOC(type, size)	\
	(type *) malloc(sizeof(type) * (size))	/* Ultrix mod - fsg */
	...
	u_long replylen;	
{
	...
	if (victim->cache_reply != NULL) {
		...
		newbuf = malloc(su->su_iosz);	/* Ultrix mod - fsg */
		if (newbuf == NULL) {
