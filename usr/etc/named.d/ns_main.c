#ifndef lint
static	char	*sccsid = "@(#)ns_main.c	4.3	(ULTRIX)	2/14/91";
#endif lint
 ...
 *
 * 17-Jul-90	sue
 *	Added mallocs so that all named args can be seen as is on a ps.
 */
...
#else
char	*debugfile = "/usr/var/tmp/named.run";
#endif
...
int	needzoneload = 0;		/* need to reload secondary zone(s) from
						xfer output file */
...
#ifdef NEW_SIG
int	need_rereadzones = 0;		/* Need to check to see if the source
					 * files have changed for the primary
					 * or the zones of a secondary need
					 * to be pulled again.
					 */
#endif NEW_SIG
#ifdef ULTRIXFUNC
int	continuemaint = 0;		/* If continuemaint == 1, there is at
					 * least one zone that is being slowly
					 * loaded.  So, keep loading the zone.
					 */
#endif ULTRIXFUNC
	...
	char *argv[], *envp[];
{
	...
	extern void endxfer();
#ifdef NEW_SIG
	extern int setIncrDbgFlg(), setNoDbgFlg(), sigprof(), rereadzones();
#else NEW_SIG
			...
			while (*++arg)
				...
				case 'a':
					...
					if(*ptr != '.')
						...
						if ((authentype_c = (char *)malloc(i)) == NULL) {
#ifdef DEBUG
							if (debug >= 5)
								fprintf(ddt,"named: malloc error\n");
#endif
							syslog(LOG_ERR, "named: Out Of Memory");
							exit(12);
						}
						strncpy(authentype_c, authen_ptr, i);
						if ((authenver_c = (char *)malloc(strlen(ptr + 1))) == NULL) {
#ifdef DEBUG
							if (debug >= 5)
								fprintf(ddt,"named: malloc error\n");
#endif
						...
						strcpy(authenver_c, ptr + 1);
					}

					if((defauthentype=m_str_val(m_credtype,
						     NUMMAPCREDTYPE,
						     authentype_c))== RET_BAD)
						usage();
					_res.defauthtype = defauthentype;

					if((defauthenver=m_str_val(m_credval,
						     NUMMAPCREDVAL,
						     authenver_c)) == RET_BAD)
						usage();
				...
				case 'b':
					if (--argc <= 0)
						usage();
					if ((bootfile = (char *)malloc(strlen(*++argv))) == NULL) {
#ifdef DEBUG
						if (debug >= 5)
							fprintf(ddt,"named: malloc error\n");
#endif
...
*/
	...
	if(netsafe) {
		...
		local_addr = *((u_long *)temp_ent->h_addr);

		if((ptr = index(localhost, '.')) != (char *)0)
			*ptr = '\0';

		if((result = krb_svc_init("named", localhost, NULL, 0, NULL,
				"/var/dss/kerberos/tkt/tkt.named")) != RET_OK){
...
#ifdef NEW_SIG
	(void) signal(SIGXFSZ, rereadzones);
#endif NEW_SIG
...
#if 0
/*..rjg..
 *	Ultrix has a bigger FD_SETSIZE than BERK
 *		syslog(LOG_ERR, "Return from getdtablesize() > FD_SETSIZE");
 */
#endif
		syslog(LOG_INFO, "..(new) named started..");
#ifdef DEBUG
		if (debug)
		...
		*/
		...
		for (dqp = datagramq; dqp != QDATAGRAM_NULL;
		    ...
		    if (FD_ISSET(dqp->dq_dfd, &tmpmask))
			     ...
			     */
			    gettime(&tt);
/* I changed PACKETSZ below to BUFSIZ below.  This could affect transfers over
	gateways.  Fix this problem with TCP and malloc */
			    ns_req(buf, n, BUFSIZ, QSTREAM_NULL, &from_addr,
				    dqp->dq_dfd);
		        }
		}
		/*
...
usage()
{
	fprintf(stderr, "Usage: named [-n] [-a auth_type.auth_ver] [-d #] [-p port] [{-b} bootfile]\n");
	exit(1);
}
...
#ifdef DEBUG
	if(debug)
	  fprintf(ddt,"Reread of zones begins.\n");
#endif
...
**
**  this is setup to fork and exit to drop to /usr/var/tmp/gmon.out
