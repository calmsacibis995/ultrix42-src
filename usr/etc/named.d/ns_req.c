#ifndef lint
static	char	*sccsid = "@(#)ns_req.c	4.4	(ULTRIX)	4/11/91";
#endif lint
 ...
 *
 * 09-Jun-88	logcher
 *	Removed incorrect comment regarding Hesiod integration.
 *
 * 11-Nov-88	logcher
 ...
 *
 * 09-Apr-90	sue
 *	Added to the if statement to allow a zone transfer of IN 
 *	information from a netsafe machine to a non-netsafe machine.
 *
 * 06-Aug-90	sue
 *	If a query was answered between the time of the initial connect
 *	for a zone transfer and the answer to the zone transfer, the
 *	zone transfer failed.  This produced spurious error messages in
 *	syslog.  The change is needed because the address of the
 *	clients that use TCP sockets was not correctly associated with
 *	the query from the client.  Changed all references of from_addr
 *	to from.
 */
...
extern int errno;
#ifdef AUTHEN
extern struct netinfo netloop;
extern int netsafe;
extern int defauthentype;
extern int defauthenver;
extern u_long local_addr;
	...
	int dfd;
{
	...
	int class_authenrr;
	AUTH_DAT ns_ad;
	int auth_type = AUTH_NONE;
	int auth_version = ONE;
	int query_to_aquery;
	int cred_len;
	char *dnptr;
	int err;
	...
	*dpp++ = msg;
	...
	case QUERY:
		...
		if (ntohs(hp->qdcount) != 1 ||
		    hp->ancount || hp->nscount ||  (ntohs(hp->arcount) != 0 &&
						   ntohs(hp->arcount) != 1)) {
#else AUTHEN
		if (ntohs(hp->qdcount) != 1 ||
		...
		*dpp = NULL;
#ifdef AUTHEN
		if ((n = dn_expand(msg, eom, cp, query_name,
				   sizeof(dnbuf))) < 0) {
#else AUTHEN
...
#ifdef AUTHEN
		dname_authenrr = query_name;
		type_authenrr = type;
		class_authenrr = class;
		if (hp->opcode == AQUERY) {
			GETSHORT(auth_type, cp);
			GETSHORT(auth_version, cp);		
			...
			if (auth_type == AUTH_KRB && auth_version == ONE) {
				...
				if((cred_len = res_rdl_krbcred(auth_version,
						&ns_ad,
						hp, cp, eom - cp,
						dnptr, type, class,
						&addr
		...
		} else if(netsafe && class == C_HS) {
			dnptr = query_name;
			res_dotname_rmhead(&dnptr);
			dnptr = res_dotname_head(dnptr);

			if(from->sin_addr.s_addr == netloop.my_addr.s_addr
			   && strcmp(dnptr, "auth")) {
				eom += 2 * sizeof(u_short);
				msglen += 2 * sizeof(u_short);
				PUTSHORT(defauthentype, cp);
				PUTSHORT(defauthenver, cp);		
				auth_type = defauthentype;
				auth_version = defauthenver;
				hp->opcode = AQUERY;
				query_to_aquery = 1;
				free(dnptr);
			} else {
...
#ifdef AUTHEN
		hp->arcount = 0;
		bcopy(query_name, dnbuf, sizeof(dnbuf));
		ocp = cp;
#endif AUTHEN
try_again:
#ifdef DEBUG
...
#ifdef AUTHEN
		if(cname) {
			n = finddata(np, class, type, hp, &dname,
				&buflen, &count, auth_type, auth_version,
				&junk, &junk);
		} else {
		...
		if (type == T_AXFR) {
			...
			if(hp->opcode == AQUERY) {
				...
				/*
				 * If not AQUERY and netsafe is on,
				 * reject class Hesiod xfers
				 */
				if(netsafe && class != C_IN) {
					hp->rcode = AUTHEN;
					dname_authenrr = query_name;
					type_authenrr = type;
					class_authenrr = class;
			...
			startxfr(qsp, np,
				 hp->opcode == AQUERY ?  1 : 0,
				 &ns_ad,
				 auth_type,
				 auth_version,
				 msg,
				 cp - msg, cp - cred_len, dname_authenrr,
				 type_authenrr, class_authenrr);
#else
...
fetchns:
...
#ifdef AUTHEN
				    dname_authenrr = ans_name;
				    getname(np, ans_name, sizeof(ans_name));
				    type_authenrr = (int)nsp[0]->d_type;
				    class_authenrr = (int)nsp[0]->d_class;
#endif AUTHEN
				}
			}
			goto finish;
...
#ifdef AUTHEN
			msglen = res_mkquery(hp->opcode, dname, class, type,
		...
		n = ns_forw(nsp, msg, msglen, from, qsp, dfd, &qp,
			    ((query_to_aquery == 1) ? QUERY: hp->opcode),
			    ((query_to_aquery == 1) ? AQUERY: hp->opcode),
			    auth_type, auth_version, auth_type, auth_version,
			    ns_ad, dname, type, class);
