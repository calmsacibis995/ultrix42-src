#ifndef lint
static char *sccsid = "@(#)utilities.c	4.2	ULTRIX	10/15/90";
#endif
/************************************************************************
 *									*
 ...
 *
 * 15 Oct 90  -- dws
 *	Fixed looping on '-b' invocations.  Added code for updating
 *	proto sblock.
 *
 ************************************************************************/
	...
	int fix;
{
	...
	dino = *dp;
	for (ap = &dino.di_db[1]; ap < &dino.di_db[NDADDR]; ap++)
		if (*ap)
			if (fix)
				*ap = 0;
	                else
				needs_attention++;
	for (ap = &dino.di_ib[0]; ap < &dino.di_ib[NIADDR]; ap++)
		if (*ap)
			if (fix)
				*ap = 0;
	                else
	...
	register BUFAREA *bp;
{
	...
	/* 
	 * Sync up proto sblock
	 */
	if (bflag != 0 && bp->b_bno == SBLOCK) {
		bp->b_bno = fsbtodb(&(sblock), cgsblock(&(sblock), sblock.fs_ncg-1));
                bwrite(fcp, bp->b_un.b_buf, bp->b_bno, (long)bp->b_size);
        }
}
...
ckfini()
{
	...
	if (sblk.b_bno != SBLOCK) {
		...
		sbdirty();
		flush(&dfile, &sblk);
		bflag=0;	/* Don't use alternate in next pass */
	}
	flush(&dfile, &inoblk);
	flush(&dfile, &cgblk);
