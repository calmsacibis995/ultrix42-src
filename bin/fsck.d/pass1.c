#ifndef lint
static char *sccsid = "@(#)pass1.c	4.2	ULTRIX	1/22/91";
#endif
/************************************************************************
 *									*
	...
	for (c = 0; c < sblock.fs_ncg; c++) {
		for (i = 0; i < sblock.fs_ipg; i++, inumber++) {
			if (inumber < ROOTINO)
				continue;
			dp = ginode(inumber);
			if ((dp->di_spare[0]) || (dp->di_spare[1]) ||
			    (dp->di_spare[2]) || (dp->di_spare[3])) {
				pfatal("SPARE FIELDS OF INODE CONTAINED VALUES I=%u",
					    inumber);
				if (reply("CLEAR") == 1) {
					zapino(dp);
					inodirty();
			...
			if (!ALLOC(dp)) {
				...
				if (dp->di_flags) {
					pwarn("UNALLOCATED INODE HAS UNKNOWN FLAG I=%u",
						inumber);
					if (preen)
						printf(" (FIXED)\n");
					else if (reply("FIX") == 0)
			 ...
			 */
				...
				if (debug) {
					printf("pass1: inode %d is FASTLINK\n",
						inumber);
					printf("di_flags = 0x%x di_size = %d\n",
						dp->di_flags, dp->di_size);
				}
