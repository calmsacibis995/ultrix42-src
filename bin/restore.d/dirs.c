...
# ifndef lint
static char *sccsid = "@(#)dirs.c	4.2    (ULTRIX)        9/7/90";
# endif not lint
 ...
 *
 * 16 Aug 90 -- carito
 *	Adhoc modification to extractdirs() to detect empty dumpset and
 *      print error message instead of core dumping.
 *
 *  8 Sep 86 -- fries
	...
	int genmode;
{
	...
	for (;;) {
		...
		/* Adhoc mechanism to detect empty dump set 16 Aug 90 */
		if ( ip == 0 ) {
		  fprintf (stderr,
		      "restor: appears to be empty - Nothing to restore!\n");
		  done (1);
		}
