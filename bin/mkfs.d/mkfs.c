#ifndef lint
static char *sccsid = "@(#)mkfs.c	4.3	ULTRIX	12/6/90";
#endif
 ...
 *
 *	dws -- 05-Dec-90
 *	Fixed usage line and added arg count check.
 *
 *	prs -- 22-May-90
 *	Added back check which calculated fs_size to be a multiple of 
 *	the file system block size.
 *
 * 	prs - 05-June-90
 *	Ensured that "ra" disks on mips processors are made
 *	with a rotational delay of 4ms.
 *
 *	prs - 15-Jul-88
 *	Added code to initcg() and fsinit() that writes random numbers
 *	into the generation number field of on-disk inodes.
 *
 *      Paul Shaughnessy, 09-May-88
 * 006- Changed writing of the clean byte in the superblock from
 *      the fs_fmod field to fs_clean.
 *
 * 	Joe Amato, 12-May-87
 * 005- modified to use new getmountent library routine
 *
...
int	rotdelay = ROTDELAY;	/* rotational delay value */
int	Nflag;
daddr_t	alloc();
char *Usage =
  "Usage: mkfs [ -N ] special size [ nsect ntrak bsize fsize cpg minfree rps nbpi opt ]\n";
...
#ifdef mips
	/*
	 * The following is used to override the default for 'rotdelay' on
	 * mips based platforms. The default is zero which is good for scsi
	 * and msi based disks. The real 'ra' disks however really need a
	 * rotdelay of 4.
	 */
	if (ioctl(fso, DEVIOCGET, (char *)&devget) >= 0) {
		if ((devget.device[1] == 'A')  || (devget.device[1] == 'a'))
			rotdelay = RA_ROTDELAY;
	}
#endif mips
...
next:
	...
	sblock.fs_lastfsck = utime.tv_sec;
	sblock.fs_deftimer = sblock.fs_cleantimer = FSCLEAN_TIMEOUTFACTOR;
	wtfs(SBLOCK, SBSIZE, (char *)&sblock);
	/*
	 * 004 - Duplicate super blocks should not have the clean byte set !
	 */
	sblock.fs_clean = sblock.fs_cleantimer = 0;
	...
	int cylno;
{
	...
	register struct timeval timeval;
	register struct timezone timezone;
	...
	/*
	 * Set seed for srandom to be a number that would be difficult
	 * to duplicate.
	 */
	seed = getpid();
	srandom(timeval.tv_sec + timeval.tv_usec + seed);
	/*
	 * Lets be paranoid.
	 */
	while (seed--)
		random();
	/*
	 * lseek to offset where the inode block is located.
	 */
	lseek(fso, fsbtodb(&sblock, cgimin(&sblock, cylno)) * DEV_BSIZE, 0);
	/*
	 * Zero out inode about to be written to disk.
	 */
	zinop = zino;
	bzero(zinop, MAXBSIZE);
	/*
	 * for each inode in the cylinder group, write out a zero inode,
	 * with a random number for a generation number.
	 */
	bufsize = INOPB(&sblock) * sizeof (struct dinode);
#ifdef DEBUG
	if (bufsize != sblock.fs_bsize)
		printf("bsize = %d fs_bsize = %d\n", bufsize, sblock.fs_bsize);
#endif
	for (i = 0; i < sblock.fs_ipg; ) {
		for (zinop = zino; zinop < &zino[INOPB(&sblock)]; zinop++) {
			zinop->di_gennum = random();
			i++;
		}
		if (!Nflag) {
			if (write(fso, (char *)zino, bufsize) != bufsize)
...
#define MSIZE (NMOUNT*sizeof(struct fs_data))
	...
	if ((ret = getmountent(&loc, mountbuffer, NMOUNT)) == 0) { /* 005 */
		perror("getmount");
		exit(3);
	}
