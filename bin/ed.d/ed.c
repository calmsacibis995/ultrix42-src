#ifndef lint
static char *sccsid = "@(#)ed.c	4.2 (Ultrix) 10/12/90";
...
/*
 ...
 ~~~~~~~~~~~~~~~~~~~
13	07-Mar-89, Teoman Topcubasi
	Fixed the quoted delimiter problem.
	
12	07-Mar-89, Teoman Topcubasi
	Changed file name length to be MAXPATHLEN

11	09-Jun-88, Mark Parenti
	Changed signal handlers to void.

10	28-Mar-88, Teoman Topcubasi
	Fixed a bug with ed relating to file truncation whenever
	a file was > 128K.

09	01-Mar-88, Andy Gadsby, Teoman Topcubasi
	Eight bit cleaned ed while still allowing quoting.

08      19-Jan-88, Mark Parenti
        Remove signal() definition as it is defined in sys/signal.h
        Change signal handlers to void.

07	31-Dec-87, Teoman Topcubasi
	modified Slink so it handles soft links properly.
	(decided not to use reslink function after all)
 
06	11-Aug-87, Teoman Topcubasi, Chas Bennett
	allow editing of files referenced by symbolic links where
	the link and the file are on separate file systems.
 
05	16-Jul-87, Teoman Topcubasi
	retracted the changes that were made in mod #03 
 
04	15-Jul-87, Teoman Topcubasi
	installed recognition of soft links
	See SMU-01601
 
03	13-Jul-87, Teoman Topcubasi
	Changed regexp handling from static to dynamic buffer allocation.
	See ICA-05890
...
char	*msgtab[] =
{
	...
	"Caution: 'w' will truncate file; File is too big", /* 63 */
	0
};
	...
	*argv = p1 + 1;
	...
	while (argc > 1 && **argv=='-') {
		...
		case 'p':
			...
			argc--;
			if(!*argv) {
				/* use write, not printf (keep it small) */
				/* printf("ed: -p arg missing\n"); */
#define ARG_MESS "ed: -p arg missing\n"
...
swch:
	...
	case 'w':
		...
		if((zero != dol) && (addr1 <= zero || addr2 > dol))
			error(15);
		filename(c);
   /* 10 */	if (nowrite && (!strcmp(file,savedfile))) {
   /* 10 */	    fchange = 0;
   /* 10 */	    error(63);
		}
		if(Xqt) {
		 ...
		 ******* JSD: ******/
			...
			if (Fl.st_nlink == 1 && (Fl.st_mode & S_IFMT) == S_IFREG) {
				...
				if ((io = creat(funny, Fl.st_mode)) >= 0) {
					...
					exfile();
					lstat(file, &Lc);
	/* 04 */			if ((Lc.st_mode & S_IFMT) == S_IFLNK)
					{
	/* 06 */				if( Slink( funny, file, &Fl )
							< 0 )
							error(10);
...
/* 13 */	if (c==seof && !q)	/* Check for quoting */
			break;
		*p++ = c;
		if (p >= &rhsbuf[LBSIZE])
			error(38);
