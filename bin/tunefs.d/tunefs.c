#ifndef lint
static char *sccsid = "@(#)tunefs.c	4.1      ULTRIX  7/2/90";
#endif lint
...
again:
	...
	for (; argc > 0 && argv[0][0] == '-'; argc--, argv++) {
		for (cp = &argv[0][1]; *cp; cp++)
			...
			case 'c':
				name = "default clean byte timeout factor";
				if (argc < 1)
					fatal("-c: missing %s", name);
				argc--, argv++;
				i = atoi(*argv);
				if (i < 0 || i > 255)
					fatal("%s: bad %s", *argv, name);
				fprintf(stdout,
					"%s changes from %d%% to %d%%\n",
					name, sblock.fs_deftimer, i);
				sblock.fs_deftimer = i;
				continue;
...
usage:
	...
	fprintf(stderr, "\t-c default clean byte timeout factor\n");
	exit(2);
}
